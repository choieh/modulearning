		...
		success: function(data){
			...
			if(pageMode == 'mobilePage'){
				if (window.location.hostname.split('.')[0] == 'm') {
					top.location.href='../m/login.php'
					return;
				}

				if (window.location.hostname.split('.')[0] != 'm') {
					if(subDomain === 'ourhome') {
						top.location.href='../mSC/loginOurHome.php'
						return;
					}
					top.location.href='../mSC/login.php'
					return;
				}
...
function viewMenu(){
	...
	menus += '<li onClick="top.location.href=\'history.html\'">학습종료과정 (결과확인)</li>';
	menus += '<li onClick="top.location.href=\'board.html\'">공지사항</li>';
	//menus += '<li onClick="top.location.href=\'http://modulearning.kr/main/\'">PC화면 보기</li>';
	menus += '</menu>';
...
function joinMenu(){
	...
	menus += '<menu>';
	menus += '<li>Menu<button type="button" onClick="closeMenu()"><img src="../images/mobile/btn_hide.png" alt="닫기"></button></li>';
	menus += '<li onClick="top.location.href=\'m_study.php?lectureStart='+lectureStart+'&lectureEnd='+lectureEnd+'&contentsCode='+contentsCode+'&oriID='+oriID+'\'">학습진행</li>';
	menus += '<li onClick="top.location.href=\'m_board.php?lectureStart='+lectureStart+'&lectureEnd='+lectureEnd+'&contentsCode='+contentsCode+'&oriID='+oriID+'\'">공지사항</li>';
	menus += '</menu>';
	//menus += '<button onClick="logOut()">로그아웃</button>';
	menus += '</div></nav>';
	$('body').append(menus);
