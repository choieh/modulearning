...
ini_set("display_errors", 1);
    ...
    $objPHPExcel = new PHPExcel(); 
		...
		$sheetIndex ->setCellValue('A2','홈페이지 : http://modulearning.kr / 전화 : 02-6494-2010 / 팩스 : 02-6008-2012 / 이메일 : study@nayanet.kr')
								->mergeCells('A2:L2')
								->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$sheetIndex ->getCell('A2')
								->getHyperlink()
								->setUrl('http://modulearning.kr');
		...
		$sheetIndex ->setCellValue('J4','대규모 1000인 이상')
				...
				while($rs = mysql_fetch_array($result)) {
					...
					$sheetIndex ->setCellValue('L'.$b, "클릭보기")
											->getCell('L'.$b)
											->getHyperlink()
											->setUrl('http://modulearning.kr/lecture/?seq='.$rs[seq]);
