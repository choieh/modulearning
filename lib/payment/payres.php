<?php
    ...
    if ($xpay->TX()) {
		...
		$orderNum = date("ymd").substr(time().md5(microtime()),0,23);
		$RRN01 = $_POST['RRN01'];
		$RRN02 = $_POST['RRN02'];

		if($serviceType == "0" || $serviceType == "1" ) { // 사업주 훈련과정인 경우 주민등록번호 저장
			...
			$queryQ =  "INSERT INTO nynOrder SET
										orderNum='".$orderNum."', 
										serviceType='".$serviceType."', 
										cancelLimitDate='".$studyStart."', 
										orderName='".$LGD_PRODUCTINFO."', 
										userID='".$_SESSION[loginUserID]."', 
										orderDate='".$inputDate."', 
										contentsCode='".$contentsCode."',
										lectureStart='".$studyStart."', 
										lectureEnd='".$studyEnd."', 
										educationPrice='".$LGD_AMOUNT."', 
										orderTotalPrice='".$LGD_AMOUNT."', 
										LGD_TID='".$LGD_TID."',
										LGD_OID='".$LGD_OID."',
										orderType='C',
										orderStatus='Y'";
			...
			?>
				<script>
					alert("신청되었습니다. 신청확인 페이지로 이동합니다. 감사합니다.");
					top.location.href='../../study/studyOrder.php';
				</script>
			<?
			} else {
				echo $queryZ."<br>".$queryQ;
				exit;
			}
			...
			//상점내 DB에 어떠한 이유로 처리를 하지 못한경우 false로 변경해 주세요.
         	...
         	//echo "최종결제요청 결과 실패 DB처리하시기 바랍니다.<br>";            	            
					echo "결제가 실패하였습니다. 다시 시도하여 주시기 바랍니다.<br>";
        }
    }else {
        //2)API 요청실패 화면처리
        ...
        //최종결제요청 결과 실패 DB처리
        echo "최종결제요청 결과 실패 : 고객센터로 문의하여 주시기 바랍니다.<br>";            	                        
    }
?>
