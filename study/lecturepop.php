...
<script type="text/javascript">
  ...
  $.get(useApi,{'contentsCode':contentsCode},function(data){
	  ...
	  $.each(data.contents, function(){
		  ...
		  views += '<h2>총 <strong>'+this.chapter+'</strong>차시 교육과정</h2>';
		  ...
		  if (this.serviceType == 1){  // 환급 과정일때만 평가 항목 출력
			  ...
			  views += '</tr><tr>';
				if(this.totalPassReport != 0){
					views += '<td colspan="3">반영된 평가, 과제 점수 합산 <strong>'+this.passScore+'</strong>점 이상 (단, 최종평가 '+this.passTest+'점, 과제 '+this.passReport+'점 미만 시 과락 적용)</td>';
				}else{
					 views += '<td colspan="3">반영된 평가 점수 합산 <strong>'+this.passScore+'</strong>점 이상 (단, 최종평가 '+this.passTest+'점 미만 시 과락 적용)</td>';
				}
			  views += '</tr></table>';
