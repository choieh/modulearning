                        ...
                        <ul class="video-tab-in">
                            <li>
                                ...
                                <ul class="tab-in-txt" id="studyInfoUi">
                                    <li><p>총시간 <span id="passTime">00:00:00</span></p></li>
                                    <li><p>학습시간 <span id="studyTime">00:00:00</span></p></li>
                                    <li><p>총진도 <span id="totalProgress">0%</span></p></li>
                                    <li><p>학습진도 <span id="progress">0%</span></p></li>
                                </ul>
...
<script>
        ...
        const data = await fetch(studyInfoApi + window.location.search)
            ...
            .then(data => {
                if (data.result == 'success') {
                    ...
                    if (['1', '2'].includes(String(param.serviceType)) && progressData.progress.chapter % 8 == 1) {
                        if (checkIfMobile()) {
                            sessionStorage.setItem('mobileLink', window.location.href);
                        }

                        const certInfo = sessionStorage.getItem(param.contentsCode + '_' + progressData.progress.chapter);
                    ...
                    document.getElementById('side_title').innerText = progressData.progress.chapter + '차시. '
                    ...
                    if (param.serviceType == '5' && progressData.study.testStatus == 'C') {
                        document.getElementById('studyInfoUi').insertAdjacentHTML('beforeend', '<li><p>평가점수 <span ' +
                    ...
                    if (progressData.contents.progressCheck == 'pageCheck') {
                        document.getElementById('studyRule').innerHTML = '총시간만큼 학습하고 마지막 페이지까지 수강해야<br> 학습 진도율이 100% ' +
                            '반영됩니다.'
                    }
    ...
    function renderChapter() {
        ...
        progressData.chapter.forEach(c => {
            let href = `studySeq=${param.studySeq}&contentsCode=${param.contentsCode}&lectureOpenSeq=${param.lectureOpenSeq}&userID=${param.userID}&serviceType=${param.serviceType}&chapter=${c.chapter}`;
    ...
    function renderExam() {
        ...
        if (param.serviceType == '5'
            ...
            && (['C'].includes(progressData.study.testStatus) || (progressData.study.testStatus == 'V' && progressData.study.testOverTime == 'Y'))) {
            ...
            .then(data => {
                ...
                if (data.result == 'success') {
                    if (type == 'final' && progressData.study.survey == 'N' && progressData.study.serviceType == '1') {
                        setEmonFormValue()
                        getSurveyChk(`/study/userTestPage.php?contentsCode=${param
                            .contentsCode}&lectureOpenSeq=${param.lectureOpenSeq}&like=${type}&studySeq=${param.studySeq}&serviceType=${param.serviceType}`)
                    } else {
                        window.location.replace(`/study/userTestPage.php?contentsCode=${param
                            .contentsCode}&lectureOpenSeq=${param.lectureOpenSeq}&like=${type}&studySeq=${param.studySeq}&serviceType=${param.serviceType}`)
                    }
    ...
    // 산업안전, 페이지체크 진도제어 초기화용
    function initPageObj() {
        if (progressData.progress.progress == 100) {
            for (var i = 1; i <= highestPage; i++) {
                pageObj.push({'page': i, 'highestTime': 6000});
            }
            return;
        }
        ...
        if (param.serviceType == '5' || progressData.contents.progressCheck == 'pageCheck') {
            for (var i = 1; i <= highestPage; i++) {
                if (i == currentPage) {
                    pageObj.push({'page': i, 'highestTime': progressData.progress.timeCursor});
                } else if (i < highestPage) {
                    pageObj.push({'page': i, 'highestTime': 6000});
                } else {
                    pageObj.push({'page': i, 'highestTime': 0});
                }
            }
            return;
        }
        ...
        if (param.serviceType == '5' || progressData.contents.progressCheck == 'pageCheck') {
            ...
            if (currentPage < highestPage) {
                ...
                if (!pageObj.find(p => p.page == searchPage)) {
                    pageObj.push({'page': searchPage, 'highestTime': Number(time)})
                } else {
            ...
            if (currentPage >= highestPage) {
                if (!pageObj.find(p => p.page == searchPage)) {
                    pageObj.push({'page': searchPage, 'highestTime': 0})
                }
            }
        }
    }
    ...
    // Kalman 필터 초기화
    const kalmanFilterLat = new KalmanFilter({R: 0.01, Q: 0.01});
    const kalmanFilterLong = new KalmanFilter({R: 0.01, Q: 0.01});
    ...
    function getLocation() {
        if (navigator.geolocation) {
            ...
            Swal.fire({
                icon: 'warning',
                title: '경고',
                text: `위치 정보 수집을 지원하지 않는 브라우저입니다.`,
                timer: 5000,
    ...
    function savePosition(position) {
        const filteredLat = kalmanFilterLat.filter(position.coords.latitude);
        const filteredLong = kalmanFilterLong.filter(position.coords.longitude);
        ...
        if (locations.length == 30) {
            const avgPosition = calculatePositionAvg();
            calculatePosition(avgPosition, {coords: {latitude: filteredLat, longitude: filteredLong}});
        }
    }
    ...
    function calculatePosition(avgPosition, position) {
        ...
        if (distance > 5) countIsMoved++;

        if (distance <= 5) countIsMoved = 0;

        if (countIsMoved >= 5) {
            if (countViolations == 0) isFirst = true;
            
            countViolations++;
            countIsMoved = 0;
        }

        if (countViolations == 1 && isFirst) {
            try {
                Swal.fire({
                    icon: 'warning',
                    title: '경고',
    ...
    function calculatePositionAvg() {
        const totalLat = locations.reduce((total, loc) => total + loc.latitude, 0);
        const totalLong = locations.reduce((total, loc) => total + loc.longitude, 0);

        const averageLat = totalLat / locations.length;
        const averageLong = totalLong / locations.length;

        const diffsLat = locations.map((loc) => (loc.latitude - averageLat) ** 2);
        const diffsLong = locations.map((loc) => (loc.longitude - averageLong) ** 2);

        const varianceLat = diffsLat.reduce((a, b) => a + b) / locations.length;
        const varianceLong = diffsLong.reduce((a, b) => a + b) / locations.length;

        const stddevLat = Math.sqrt(varianceLat);
        const stddevLong = Math.sqrt(varianceLong);

        const filteredLocations = locations.filter(location =>
            Math.abs(location.latitude - averageLat) <= stddevLat &&
            Math.abs(location.longitude - averageLong) <= stddevLong
        );

        const totalFilteredLat = filteredLocations.reduce((total, loc) => total + loc.latitude, 0);
        const totalFilteredLong = filteredLocations.reduce((total, loc) => total + loc.longitude, 0);

        const averageFilteredLat = totalFilteredLat / filteredLocations.length;
        const averageFilteredLong = totalFilteredLong / filteredLocations.length;

        return {
            avgLat: averageFilteredLat,
            avgLong: averageFilteredLong
        };
    }
