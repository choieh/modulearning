...
<div id="container">
    <div class="content lg:w-1/2 w-screen mx-auto lg:border lg:border-black bg-white lg:p-10">
        <div class="inner"></div>
    </div><!-- end_content -->
</div><!-- end_container -->
</body>
...
<script>
  const test = '<?=$_GET['test']?>';
  const serviceType = '<?=$_GET['serviceType']?>';
  let ans = []; //과제용 변수
  const chapterApi = '../api/apiStudyChapter.php';
  // const surveyApi = test ? `/api/apiSurveyTest.php?serviceType=${serviceType}` : '../api/apiSurvey.php';
  // const surveyAnswerApi = test ? '/api/apiSurveyAnswerTest.php' : '../api/apiSurveyAnswer.php';
  const surveyApi = `/api/apiSurveyTest.php?serviceType=${serviceType}`
  const surveyAnswerApi = '/api/apiSurveyAnswerTest.php'
  const testApi = '../api/apiStudyTest.php';
  const reportApi = '../api/apiStudyReport.php';
  const cautionApi = '../api/apiSiteInfo.php';
  const contentsCode = "<?= $_GET['contentsCode'] ?>";
  const lectureOpenSeq = "<?= $_GET['lectureOpenSeq'] ?>";
  let like = "<?= $_GET['like'] ?>";
  let curDate = "<?= $inputDate ?>";
  let timer = '';
  let chapterObj;
  let status = '';
  let endTime = '';
  ...
  // pc, 모바일에 따라 각각 따로 강의실로 보내기
  function backToStudyRoom() {
    window.location.replace(window.location.origin + sessionStorage.getItem('referrer'));
    return;
  }

  function hrdCheckScore(flag) {
    let eval_cd = '';
    if (chapterObj.serviceType == '1') {
      if (like == 'mid') {
        eval_cd = '04';
      } else if (like == 'final') {
        eval_cd = '02';
      ...
      fetch('../api/apiHrdScoreCheck.php', {
        method: "POST",
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: `startEnd=${flag}&type=${like}&contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&eval_cd=${eval_cd}`,
        keepalive: true
      })
    }
  }
  ...
  if (like == 'mid') {
    status = chapterObj.midStatus;
  } else if (like == 'final') {
    status = chapterObj.testStatus;
  } else if (like == 'report') {
    status = chapterObj.reportStatus;
  }
  ...
  function submitSurvey(count) { //설문조사 제출
    if (loginUserID !== '9301269272') {
      if (!validateAnswer(count)) {
        return false;
      }
    }
  ...
  function getSurvey() { // 설문조사 정보 가지고 옴
    ...
    $.get(surveyApi, {}, function (data) {
      survey += leaveBtn();
      survey += '<div class="page_title rounded-md">';
      survey += '<h3 class="title p-2 text-3xl font-bold text-center" style="letter-spacing:3px;">설문조사</h3>';
      survey += '</div>';
      ...
      $.each(data.survey, function () {
        ...
        if (this.surveyType == 'A') {
          countTypeA++;
          survey += `<ul id="sel" class="type${svct}" style="font-size:14px; padding:0px 10px;">`;
          for (let i = 1; i <= 5; i++) {
            survey += renderLi(eval('this.example0' + i), this.seq, i);
          }
        } else {
          countTypeB++;
          survey += `<textarea id="long" class="type${svct}" name="userAnswer${this.seq}"
                    style="width:100%; margin-top:5px; padding:5px; border:1px solid; border-radius:5px;
                    resize:none; height:100px;"></textarea>`;
        }
        survey += '</ul>';
        survey += '</div>';
        svct++
  ...
  function setReportExam(id, seq) { // 레포트 문제 셋팅
    ...
    if (id == 'addFile') {
      ...
      result += `<textarea class="type${seq}" name="answerText${seq}"
		style="width:100%; margin-top:5px; padding:10px 5px; border:none; resize:none; height:370px;
		border-radius:5px;" placeholder="답안작성" onpaste="alert('붙여넣기는 금지입니다.'); return false;"
		onblur="reportTypingSubmit(this, ${seq})">${ans[seq].answerText}</textarea>`;
    }
    result += '</div>';
    return result;
  }
...
//		}
  ...
  function setReport() { //과제 출력
    $.ajax({
      ...
      success: function (data) {
        ...
        result += leaveBtn();
        result += '<div class="page_title rounded-md">';
        result += `<h3 class="title bg-[#545046] text-white p-2 text-center">${data.contentsName}</h3>`;
        result += '</div>';
  ...
  function setTest(data) { // 시험 문항 출력
    ...
    result += leaveBtn();
    result += '<div class="page_title rounded-md">';
    result += `<h3 class="title bg-[#545046] text-white p-2 text-center">${contentsName}</h3>`;
    result += '</div>';
    if (like == 'final') {
