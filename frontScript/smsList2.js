...
function listAct(page){
	...
	// actionArea += '|&nbsp;<input type="radio" name="selectSearch" id="searchWriteDate" value="searchWriteDate" onChange="searchTypeSelect(this.value)" /><label for="searchWriteDate"></label>&nbsp;&nbsp;&nbsp;'
    ...
    actionArea += '<input type="text" style="width:100px;" name="searchUser">';
	actionArea += '<span>영업담당자</span>';
    actionArea += '<input type="text" style="width:100px;" name="marketerName">';
	...
	actionArea += '<div style="margin-left:10px; display:inline-block;">';
	actionArea += '<span style="display:inline-block;">수강등록일 검색</span>';
	actionArea += `<input type="date" name="searchWriteDate">`;
	actionArea += '</div>';
    ...
    actionArea += '<option value="1000">1000개</option>';
    actionArea += '</select>';
    actionArea += '</div>';
	actionArea += '<button type="submit" class="allWidth">검색 비환급(일반)</button></form>';
	actionArea += '</form>';
	actionArea += '</div>';
	...
	*/
	...
	contents += '<th style="width:105px;"><input type="checkbox" id="sendStart" onChange="checkboxAllCheck(\'sendStart\')" /><label for="sendStart">학습 시작</label></th>';
	contents += '<th style="width:105px;"><input type="checkbox" id="sendInfo" onChange="checkboxAllCheck(\'sendInfo\')" /><label for="sendInfo">평가 안내</label></th>';
	...
	contents += '<th style="width:105px;"><input type="checkbox" id="sendEnd" onChange="checkboxAllCheck(\'sendEnd\')" /><label for="sendEnd">수강종료 2일 전</label></th>';
	contents += '<th style="width:105px;"><input type="checkbox" id="sendResult" onChange="checkboxAllCheck(\'sendResult\')" /><label for="sendResult">결과 확인</label></th>';
	contents += '<th style="width:105px;"><input type="checkbox" id="sendEtc" onChange="checkboxAllCheck(\'sendEtc\')" /><label for="sendEtc">기타</label></th>';
...
function ajaxAct(sortDatas){
	...
	// sortData += sortDatas.includes('serviceType=&') ? '&serviceType=3' : '';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+sortData,function(data){ //+listCount
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
			$.each(data.study,  function(){
					...
					lists += '<td><input type="checkbox" id="sendInfo_'+this.seq+'" name="sendType[]" class="sendInfo" value="info/'+this.user.userID+'/'+this.lectureOpenSeq+'" /><label for="sendInfo_'+this.seq+'">선택하기</label><br /><button type="button" onclick="viewMessage(\'info/'+this.user.userID+'/'+this.lectureOpenSeq+'\')">내용보기</button></td>';
					...
					lists += '<td><input type="checkbox" id="sendResult_'+this.seq+'" name="sendType[]" class="sendResult" value="result/'+this.user.userID+'/'+this.lectureOpenSeq+'" /><label for="sendResult_'+this.seq+'">선택하기</label><br /><button type="button"  onclick="viewMessage(\'result/'+this.user.userID+'/'+this.lectureOpenSeq+'\')">내용보기</button></td>';
					lists += '<td><input type="checkbox" id="sendEtc_'+this.seq+'" name="sendType[]" class="sendEtc" value="etc/'+this.user.userID+'/'+this.lectureOpenSeq+'" /><label for="sendEtc_'+this.seq+'">선택하기</label><br /><button type="button"  onclick="viewMessage(\'etc/'+this.user.userID+'/'+this.lectureOpenSeq+'\')">내용보기</button></td>';
					lists += '<td onClick="globalModalAct(\'companyView\',\'\','+this.company.companyCode+')" style="cursor:pointer;">'+this.company.companyName;
					if(loginUserLevel < '5') {
            lists += '<br />영업팀장 : '+(this.marketer.highMarketer??'-');
...
function searchTypeSelect(types){
	...
	$('#searchCompany + label ~ *').remove();
	//$('#searchWriteDate + label ~ *').remove();
	...
	if(types == 'searchDate'){
 	...
 	} // else if(types == 'searchWriteDate'){
//     chageSearch += `<input type="date" name="searchWriteDate" value="${new Date().toJSON().slice(0, 10)}">`;
//   }
...
function viewMessage(values){
	...
	modalWrite += '<option value="sms">문자메시지</option>'
	modalWrite += '<option value="email">이메일</option>'
	modalWrite += '<option value="kakaoAtalk" selected="selected">카카오 알림톡</option>'
	modalWrite += '</select>';
	modalWrite += '</h1>'
	modalWrite += '<ul>'
	modalWrite += '</ul>'
