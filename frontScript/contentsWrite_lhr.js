...
function writeAct(writeSeq, Types, testType){
	...
	function writePrint(data){
		...
		if(writeType=='contentsWrite'){
			...
			writes += '<select name="contentsGrade" class="'+contentsGrade+'" onChange="alert(\''+$('select[name=contentsGrade]').val()+'\')">'+optWrite['contentsGrade']+'</select>'
			if(seq == ''){
			...
			writes += '<div class="halfDiv">'
			writes += '<h1>직무법정과정</h1>';
			writes += '<select name="dutyType2" class="'+dutyType2+'">'+optWrite['dutyType']+'</select>'
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<div class="halfDiv">'
			writes += '<h1>훈련유형</h1>';
			writes += '<select name="contentsType" class="'+contentsType+'">';
			...
			writes += '<option value="7">수강연동</option>';
			writes += '</select>';
			writes += '</div>'
			writes += '</li>';
			...
			if(previewImage == '' || previewImage == null){
				...
				writes += '<div id="previewImage" class="attachFile"><input type="hidden" name="previewImageU" value="'+previewImage+'"/><img src="'+previewImageURL+previewImage+'" style="width:100px;"><br /><button type="button" onclick="deleteFileAct(\'previewImage\')">첨부파일삭제</button></div><input type="checkbox" name="delFile01" value="Y" />';
			}
			writes += '</div>'
			writes += '<div class="halfDiv">'
			writes += '<h1>진도체크방식</h1>';
			...
			*/
			...
			writes += '<select name="limitCnt" id="limitCnt" onchange="limitPrice();">';
			writes += '<option value="3000" selected="selected">3000명</option>';
			writes += '<option value="5000">5000명</option>';
			writes += '</select>';
			...
			writes += '<div class="halfDiv">'
			writes += '<h1>공급정도</h1>';
			writes += '<input type="text" name="supply" class="price" value="'+supply+'" />';
			writes += '</div>'
			writes += '<div class="halfDiv">'
			writes += '<h1>조정계수</h1>';
			writes += '<input type="text" name="coefficient" class="price" value="'+coefficient+'" />';
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<div class="halfDiv">'
			writes += '<h1>표준훈련비</h1>';
			writes += '<input type="text" name="selfPrice" class="price" value="'+selfPrice+'" /> 원';
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<input type="text" name="selfPrice01" class="price" value="'+selfPrice01+'" /> 원';
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<input type="text" name="selfPrice02" class="price" value="'+selfPrice02+'" /> 원';
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<input type="text" name="selfPrice03" class="price" value="'+selfPrice03+'" /> 원';
			writes += '</div>'
			writes += '</li>';
			...
			writes += '<div class="halfDiv">'
			writes += '<h1>인정만료일</h1>';
			writes += '<div class="datePicker"><input type="text" name="contentsAccredit" class="cal"  value="'+contentsAccredit+'" readonly="readonly" /> ~ </div>';
...
//			writes += '<select name="contract" class="'+contract+'">'+optWrite['contract']+'</select>';
			...
			writes += '<option value="C">도입</option>';
			writes += '<option value="B">임대</option>';
			writes += '<option value="A">자체</option>';
			writes += '</select>';
			writes += '</div>'
			writes += '<div class="halfDiv">'
			...
			if(writeType=='chapterWrite'){
				$.each(data.chapter,function(){
					...
					if(data.sourceType=='book') {
						...
						var mp4Link = this.player+'/player/mp4_player_admin.php?contentsCode='+contentsCodes+'&chapter='+this.chapter;
						if(data.sourceType == 'mp4') {
							chapterLists += '<button type="button" onClick="window.open(\''+mp4Link+'\',\'학습창\',\'top=0,left=0,location=yes,menubar=no,status=no,titlebar=yes,toolbar=no,scrollbars=no,resizeable=no\',\'Study\')">PC</button>&nbsp;/&nbsp;';
						} else {
