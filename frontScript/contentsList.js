...
function listAct(page) {
    ...
    $.get(categoryApi, {'value01': 'lectureCode'}, function (data) {
        ...
        actionArea += '<span>CP수수료</span>';
        actionArea += '<select name="commission">';
        actionArea += '<option value="">전체</option>';
        actionArea += '<option value="0">0</option>';
        actionArea += '<option value="10">10</option>';
        actionArea += '<option value="20">20</option>';
        actionArea += '<option value="30">30</option>';
        actionArea += '</select>&nbsp;';
        ...
        actionArea += '<span style="margin-left:20px; margin-top:5px;">파일현황</span>'
        actionArea += '<select name="fileState" onchange="searchAct()">';
        actionArea += '<option value="">전체</option>';
        actionArea += '<option value="1">원본</option>';
        actionArea += '<option value="2">백업</option>';
        actionArea += '<option value="3">복사</option>';
        actionArea += '<option value="4">기타</option>';
        actionArea += '</select>';
        ...
        actionArea += '</select>';
        actionArea += '<span>메모</span>';
        actionArea += '<input type="text" name="memo">'
        actionArea += '</div>&nbsp;';
        ...
        */
        ...
        contents += '<th style="width: 60px">과정유형</th>';
        contents += '<th>과정등급/코드/과정명</th>';
        contents += '<th style="width:100px">심사차수</th>';
        contents += '<th>메모</th>';
        contents += '<th style="width:120px;">총차시/교육시간</th>';
        contents += '<th>CP수수료</th>';
        contents += '<th style="width:280px;">교육비(최근 변동 조정계수 기준)</th>';
        contents += '<th style="width:130px;">HRD/e-simsa 코드</th>';
        contents += '<th style="width:60px;">모바일</th>';
        contents += '<th style="width:60px;">공개여부</th>';
        /*if(loginUserLevel <= 3) {
        ...
        contents += '<th style="width:60px;">파일현황</th>';
...
function ajaxAct(sortDatas, sort01, sort02) {
    ...
    var listAjax = $.get(useApi, 'page=' + page + '&list=' + listCount + sortData + sort01s + sort02s, function (data) {
        ...
        var BBSListBg = '';
        var i = totalCount;
        let fileStates = [null, '원본', '백업', '복사', '기타'];
        //alert(page);
        if (page != 1) {
        ...
        if (totalCount != 0) {
            $.each(data.contents, function () {
                ...
                if (this.previewImage == null || this.previewImage == '') {
                    ...
                    lists += '<br /><img src="../attach/contents/' + encodeURIComponent(this.previewImage) + '" style="width:90px; height:70px; border:1px solid;">';
                }
                lists += '</td>';
                ...
                if (this.contentsType == '3') {
                    lists += '[표준 훈련비] <br>' + toPriceNum(this.selfPrice) + '원</br>';
                    lists += '[우선지원 지원금 | 자부담] <br>' + toPriceNum(this.rPrice01) + '원 | ' + toPriceNum(this.selfPrice01) + '원</br>';
                    lists += '[1000인 미만 지원금 | 자부담] <br>' + (toPriceNum(this.rPrice02)) + '원 | ' + (toPriceNum(this.selfPrice02)) + '원</br>';
                    lists += '[1000인 이상 지원금 | 자부담] <br>' + (toPriceNum(this.rPrice03)) + '원 | ' + (toPriceNum(this.selfPrice03)) + '원';
                } else {
                    lists += '[표준 훈련비] <br>' + toPriceNum(this.contentsPrice.selfPrice) + '원</br>';
                    lists += '[우선지원 지원금 | 자부담] <br>' + toPriceNum(this.contentsPrice.rPrice01) + '원 | ' + toPriceNum(this.contentsPrice.selfPrice01) + '원</br>';
                    lists += '[1000인 미만 지원금 | 자부담] <br>' + (this.contentsType == '4' ? '0' : toPriceNum(this.contentsPrice.rPrice02)) + '원 | ' + (this.contentsType == '4' ? '0' : toPriceNum(this.contentsPrice.selfPrice02)) + '원</br>';
                    lists += '[1000인 이상 지원금 | 자부담] <br>' + (this.contentsType == '4' ? '0' : toPriceNum(this.contentsPrice.rPrice03)) + '원 | ' + (this.contentsType == '4' ? '0' : toPriceNum(this.contentsPrice.selfPrice03)) + '원';
                }

                if (this.contentsType != '3') {
                    lists += `<br><button type="button" onclick="getContentsPrice('${this.contentsCode}', ${this.contentsType})">연도별 교육비</button>`;
                }
                ...
                lists += '<td>';
                if (this.fileState != 0 && this.fileState != null)
                    lists += fileStates[this.fileState];
                else
                    lists += '-';
                lists += '</td>';
...
function createPriceDialog() {
    ...
    let template = `
        ...
        <button type="button" style="border:1px solid black; padding: 5px; width: 100px;" onclick="closeDialog()">닫기</button>
        </div>
    ...
    `;
    dialog.insertAdjacentHTML('beforeend', template)
...
function getContentsPrice(contentsCode, contentsType) {
    // if (loginUserID != 'zkfmak3785') {
    //     alert('제작중 입니다.')
    //     return;
    // }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == 'success') {
                ...
                data.data.forEach( e => {
                    inner += '<tr>';
                    inner += `<td>${e.applyDate}</td>`;
                    inner += `<td>${toPriceNum(e.selfPrice)}원</td>`
                    inner += `<td>${toPriceNum(e.rPrice01)}원</td>`
                    inner += `<td>${toPriceNum(e.selfPrice01)}원</td>`
                    inner += `<td>${contentsType == 4 ? 0 : toPriceNum(e.rPrice02)}원</td>`
                    inner += `<td>${contentsType == 4 ? 0 : toPriceNum(e.selfPrice02)}원</td>`
                    inner += `<td>${contentsType == 4 ? 0 : toPriceNum(e.rPrice03)}원</td>`
                    inner += `<td>${contentsType == 4 ? 0 : toPriceNum(e.selfPrice03)}원</td>`
                    inner += '</tr>';
                })
