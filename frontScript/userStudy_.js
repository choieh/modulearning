...
function studyPop(obj,chapter,types,captchChk){
	...
	$.get(chapterApi,{'contentsCode':checkCode,'lectureOpenSeq':checkOpenSeq},function(data){
		...
		if(chapter > midTerm && (data.midStatus != 'Y' && data.midStatus != 'C') && (eval(data.serviceType)== 1 || eval(data.serviceType)==9 )){
			...
			} else if(runningForm.children('input[name="sourceType"]').val() == 'html5'){
				popupAddress = '/study/study_view.html?studySeq='+checkSeq+'&contentsCode='+checkCode+'&chapter='+chapter+'&lectureOpenSeq='+checkOpenSeq+'&check='+types;
				var studyPopOpen = window.open(popupAddress,"studyWindow","width=850,height=800,top=0,left=0,location=yes,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","kiraedu")
