...
function agreeActEduCenter() {
	...
	$.get('../api/apiSiteInfo.php', {}, function (data) {
		...
		agreeWrite += '<div>' + data.privacy + '</div>';
		agreeWrite += '</div>';
		agreeWrite += '<div class="btnArea"><button type="button" onclick="checkAgreeEduCenter()">회원가입 계속하기</button></div>';
		$('#contentsArea').removeAttr('class')
		$('#contentsArea').addClass('agreementPage')
		$('#contentsArea').html(agreeWrite);
	})
...
function checkAgreeEduCenter() {
	if ($('.agreementPage input[name="checkAgree"]').prop('checked') != true) {
		alert('이용약관에 동의해주세요')
	} else if ($('.agreementPage input[name="checkPrivate"]').prop('checked') != true) {
		alert('개인정보 보호정책에 동의해주세요')
	} else {
		viewMemberInfoEduCenter();
	}
}
...
function viewMemberInfoEduCenter() {
	if (loginUserID != '') {
		var writeAjax = $.get(useApi, {'userID': loginUserID}, function (data) {
			$.each(data.member, function () {
				seq = this.seq;
	...
	function writePrint() {
		...
		if (department == '') {
			writes += `<li class="mustCheck"><h1>회사명</h1><input type="text" name="department" class="name" maxlength="50" value="${department}" /></li>`;
		} else {
			writes += `<li class=""><h1>회사명</h1><input type="hidden" name="department" value="${department}"><strong>${department}</strong></li>`;
		}

		writes += '<li class="mustCheck">';
		writes += '<h1>직급</h1>'
		writes += `<input type="text" name="rank" class="name" value="${rank}" maxlength="20" />`
		writes += '</li>'

		writes += '<li class="mustCheck">';
		writes += '<h1>직무</h1>'
		writes += `<input type="text" name="job" class="name" value="${job}" maxlength="20" />`
		writes += '</li>'
		...
		writes += '<h1>성별</h1>';
		if(loginUserID == ''){
			writes += '<select name="sex" class="'+sex+'" style="padding: 10px; font-size:14px; height: 40px;' +
				' appearance: auto">';
		...
		//휴대폰입력
		writes += '<li class="mustCheck"><h1>휴대전화</h1><select name="mobile01" class="' + mobile01 + '" style="appearance: auto">';
		writes += '<option value="010">010</option>';
		writes += '<option value="011">011</option>';
		writes += '<option value="016">016</option>';
		writes += '<option value="017">017</option>';
		...
		writes += '<li class="mustCheck"><h1>주소</h1><div class="address">';
		writes += '<input name="zipCode" class="name" type="tel" maxlength="5" id="zipCodeArea"  value="' + zipCode + '" readonly="readonly" />&nbsp;<button type="button" class="findZipCode">우편번호 찾기</button><br />';
		writes += '<input name="address01" class="subject" type="text" id="address01Area" value="' + address01 + '" /><br />';
		writes += '<input name="address02" class="subject" type="text" id="address02Area" value="' + address02 + '" /><br>';
		writes += '<strong style="font-size:14px; color: tomato">* 교재 발송을 위해 주소를 정확하게 기입해주세요. *</strong>';
		writes += '</div></li>';
...
function checkMemberForm() {
	...
	if (window.location.host.split('.')[0] == 'educenter') {
		if ($('input[name="rank"]').val() == '') {
			$('input[name="rank"]').after('&nbsp;&nbsp;<strong class="red">직급을 입력해주세요.</strong>')
		}

		if ($('input[name="job"]').val() == '') {
			$('input[name="job"]').after('&nbsp;&nbsp;<strong class="red">직무를 입력해주세요.</strong>')
		}

		if ($('select[name="sex"]').val() == '') {
			$('select[name="sex"]').after('&nbsp;&nbsp;<strong class="red">성별을 선택해주세요.</strong>')
			checkFalse++;
		}

		if ($('input[name="department"]').val() == '') {
			$('input[name="department"]').after('&nbsp;&nbsp;<strong class="red">회사명을 입력해주세요.</strong>')
			checkFalse++;
		}

		if ($('input[name="zipCode"]').val() == '' || $('input[name="address01"]').val() == '' || $('input[name="address02"]').val() == '') {
			$('input[name="address02"]').after('&nbsp;&nbsp;<strong class="red">주소를 모두 입력해주세요.</strong>')
			checkFalse++;
		}
	}
	...
	if (checkFalse == 0) {
		...
		if (confirm(typesTxt + " 하시겠습니까?")) {
			$.ajax({
				url: '../api/apiStudyCenterMember.php',
				type: 'POST',
