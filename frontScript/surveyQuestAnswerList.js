...
function listAct(page){
	...
	contents += '<th style="width:200px;">TARGET_USER_ID</th>';
	contents += '<th style="width:120px;">ANSWER_USER_ID</th>';
	contents += '<th style="width:80px;">COURSE_AGENT_PK</th>';
	contents += '<th style="width:80px;">CLASS_AGENT_PK</th>';
	contents += '<th style="width:80px;">OBJ_ANSWER_RULE_ID</th>';
	contents += '<th style="width:80px;">OBJ_ANSWER_ITEM_ID</th>';
	contents += '<th style="width:80px;">SUBJ_ANSWER</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+'&joinURL='+joinURL+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
					...
					lists += '<td>';
					if(this.TARGET_USER_ID == '' || this.TARGET_USER_ID == null){
						lists += '-'
					}else{
						lists += this.TARGET_USER_ID;;
					}
					lists += '</td>';
					lists += '<td>'+this.ANSWER_USER_ID+'</td>';
					lists += '<td>'+this.COURSE_AGENT_PK+'</td>';
					lists += '<td>'+this.CLASS_AGENT_PK+'</td>';
					lists += '<td>';
					if(this.OBJ_ANSWER_RULE_ID == '' || this.OBJ_ANSWER_RULE_ID == null){
						lists += '-'
					}else{
						lists += this.OBJ_ANSWER_RULE_ID;;
					}
					lists += '</td>';
					lists += '<td>'+this.OBJ_ANSWER_ITEM_ID+'</td>';
					lists += '<td>';
					if(this.SUBJ_ANSWER == '' || this.SUBJ_ANSWER == null){
						lists += '-'
					}else{
						lists += this.SUBJ_ANSWER;;
					}
					lists += '</td>';
