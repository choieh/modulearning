$(document).ready(function(){
	$('#datePicker > h1').bind("click", function(){
		alert('test');
	});
});
...
function CalenderAct(useType,Prints,date){
	...
	calendarWrite += '<div>';
	calendarWrite += '<button type="button" onclick="CalenderAct(\''+useType+'\',\'' +  Prints + '\', \'' + prevYear + '\')"><img src="../images/admin/btn_cal_prevYear.png" alt="이전연도" /></button>';
	...
	calendarWrite += '<button type="button" onclick="CalenderAct(\''+useType+'\',\'' +  Prints + '\', \'' + nextYear + '\')"><img src="../images/admin/btn_cal_nextYear.png" alt="다음연도" /></button>';
	calendarWrite += '</div>';
...
function CalenderAct2(useType,Prints,date){
	//참조 http://blog.kurien.co.kr/516
	//var date = date ? date : new Date();
	...
	calendarWrite += '<div>';
	calendarWrite += '<button type="button" onclick="CalenderAct2(\''+useType+'\',\'' +  Prints + '\', \'' + prevYear + '\')"><img src="../images/admin/btn_cal_prevYear.png" alt="이전연도" /></button>';
	calendarWrite += '<button type="button" onclick="CalenderAct2(\''+useType+'\',\'' +  Prints + '\', \'' + prevDate + '\')"><img src="../images/admin/btn_calprev.png" alt="이전달" /></button>&nbsp;';
	calendarWrite += '<h1>'+currentYear + '</h1>년 <h2>' + currentMonth + '</h2>월';
	calendarWrite += '<button type="button" onclick="CalenderAct2(\''+useType+'\',\'' +  Prints + '\', \'' + nextDate + '\')"><img src="../images/admin/btn_calnext.png" alt="다음달" /></button>';
	calendarWrite += '<button type="button" onclick="CalenderAct2(\''+useType+'\',\'' +  Prints + '\', \'' + nextYear + '\')"><img src="../images/admin/btn_cal_nextYear.png" alt="다음연도" /></button>';
	calendarWrite += '</div>';
	calendarWrite += '<table id="calendarTable">';
	calendarWrite += '<thead><tr><th width="14.3%">일</th><th width="14.3%">월</th><th width="14.3%">화</th><th width="14.3%">수</th><th width="14.3%">목</th><th width="14.3%">금</th><th width="14.3%">토</th></tr></thead>';
	calendarWrite += '<tbody>';
	...
	for(var i = 0; i < week; i++){
		...
		for(var j = 0; j < 7; j++, dateNum++){
			...
			if (String(yys).length < 2) yys = '0'+yys;
			if (String(mms).length < 2) mms = '0'+mms;
			if (String(dds).length < 2) dds = '0'+dds;

			var date1 = yys+'-'+mms+'-'+dds; // 오늘날짜
			var date2 = currentYear+'-'+currentMonth+'-'+dateNums; // 달력 날짜
			var diffDay = diffDays(date1, date2) // 기준날짜, 변하는 날짜, 절대값 필요시 'a'

			if(j == 2 || j == 4){ // (j == 2 || j == 4) && diffDay >= 6 <-- 기존 지난 날짜는 못하게 했을 시
				calendarWrite += '<td style="color:red;" id="'+currentYear+'-'+currentMonth+'-'+dateNums+'"><h1>' + dateNum + '</h1></td>';
			} else {
				calendarWrite += '<td style="color:gray;"><h1>' + dateNum + '</h1></td>';
			}
//			if(j == 2 || j == 4){
//				calendarWrite += '<td style="color:red;" id="'+currentYear+'-'+currentMonth+'-'+dateNums+'"><h1>' + dateNum + '</h1></td>';
//			}else{
...
function dateAct2(){
	...
	$('#datePicker').append('<p>&nbsp;&nbsp;<button type="button" class="pickerClose">닫기</button></p>')
	$('.pickerClose').click(function(){closePicker()})
	$('.todaySel').click(function(){todaySel()})
	$('#calendarTable').children('tbody').children('tr').children('td').click(function(){
		var dateSel = $(this).attr('id');
...
function pickerAct2(){
	$('.cal').bind({
		click:function(){
			...
			var calPrint = '<div id="datePicker"></div>'
			$(this).after(calPrint);
			CalenderAct2('act','datePicker',pickDate);
		}
	})
}
