...
function ajaxAct(sortDatas){
    ...
    $.get(useApi,'&page='+page+'&list='+listCount+sortData,function(data){
        ...
        if(totalCount != 0){
            $.each(data.reserveSms, function(){
				lists += '<tr>';
				lists += '<td><input type="checkbox" class="rslCheck" id="rslCheck_'+this.mt_pr+'" name="rslCheck[]" value="'+this.mt_pr+'" /><label for="rslCheck_'+this.mt_pr+'"></label></td>';
                lists += '<td>'+i+'</td>';
                lists += '<td>'+this.reg_date+'</td>';
				lists += '<td>'+this.date_client_req+'</td>';
				lists += '<td><a href="javascript:globalModalAct(\'companyView\',\'\','+this.companyCode+')">'+this.companyName+'</a></td>';
				lists += '<td><a href="javascript:globalModalAct(\'memberView\',\'\',\''+this.marketerID+'\')">'+this.marketerName+'('+this.marketerID+')</a></td>';
				lists += '<td><a href="javascript:globalModalAct(\'memberView\',\'\',\''+this.userID+'\')">'+this.userName+'('+this.userID+')</a><br/>'+this.recipient_num+'</td>';
				//lists += '<td>'+this.companyName+'<br>'+this.userName+'<br>'+this.recipient_num+'</td>';
                lists += '<td>'+this.content+'</td>';
                lists += '<td><button type="button" onClick="smsDelete(\''+this.mt_pr+'\',\''+this.messageType+'\')">삭제</button></td>';
                lists += '</tr>';
                i--;
            })
        }else{
...
function smsDelete(mt_pr,messageType){
	if(confirm('해당 정보를 삭제하시겠습니까? 삭제 후 복구하실 수 없습니다.')) {
		$.ajax({
			url: useApi,
			type:'DELETE',
			data:'smsDelete=Y&mt_pr='+mt_pr+'&messageType='+messageType,
			dataType:'text',
...
function monthDelete(){
	...
	//console.log(delData)
	if(confirm('예약문자내역을 삭제하시겠습니까? 삭제 후 복구하실 수 없습니다.')) {
		$.ajax({
			url: useApi,
			type:'DELETE',
			data:delData,
...
function selectDelete() {
	...
	//alert(messageType);
	if(confirm("선택된 예약문자내역을 삭제하시겠습니까? 삭제 후 복구하실 수 없습니다.")){
		$.ajax({
			method:'DELETE',
			url:useApi,
			dataType:'text',
