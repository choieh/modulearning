	...
	if(types != 'final'){
		...
		$.get(chapterApi,{'contentsCode':contentsCode,'lectureOpenSeq':lectureOpenSeq},function(data){
			...
			if(data.survey != 'Y'){
				...
				studyModals += '</div>';
				studyModals += '</div>';
        // surveyApi = /dhkang99|dhkang0|kiraedu99/.test(loginUserID) ? '../api/apiSurveyTest.php': surveyApi;
        surveyApi = '../api/apiSurveyTest.php';
				$.get(surveyApi+`?serviceType=${data.serviceType}`,{},function(data){
					//주의사항
...
function submitSurvey(types,contentsCode,lectureOpenSeq){
  ...
  // surveyAnswerApi = /dhkang99|dhkang0|kiraedu99/.test(loginUserID) ? '../api/apiSurveyAnswerTest.php': surveyAnswerApi;
  surveyAnswerApi = '../api/apiSurveyAnswerTest.php';
...
function certPassModal3(startDate,studySeq,contentsCode,lectureOpenSeq, serviceType){
	var studyModals = '';
	studyModals += '<div id="screenModal" style="display:none;">';
	//타이틀 노출
	studyModals += '<div class="titleArea">';
	...
	var cautionApi = $.get('../api/apiSiteInfo.php',function(data){
		...
		studyModals += '<ul><li>';
		studyModals += '<h1>본인인증</h1>';
		studyModals += `<button class="btnPhone" type="button" onclick="javscript:window.location.href='/auth/userCert.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&type=start&studySeq=${studySeq}&serviceType=${serviceType}&lectureStart=${startDate}&userID=${loginUserID}'">본인인증</button>&nbsp;&nbsp;`;
		studyModals += '<button type="button" onClick="window.open(\'http://www.niceipin.co.kr\')" class="btnIpin">민간아이핀센터 바로가기</button>&nbsp;&nbsp;';
		studyModals += '<button type="button" class="btnIpin" onClick="window.open(\'../attach/docs/한국산업인재융합교육원_민간아이핀.pdf\')">민간아이핀 발급순서 보기</button>';
		studyModals += '<br /><span class="red ipinInfo" id="ipinInfo">※ 타인 명의 휴대폰인 경우 민간아이핀을 발급 후 아이핀 인증으로 진행해 주시기 바랍니다. (아이핀/마이핀가입)</span>';
		studyModals += '</div>';
...
function certPassModal4(startDate, studySeq, contentsCode, lectureOpenSeq, userAuth, serviceType) {
	var studyModals = '';
	studyModals += '<div id="screenModal" style="display:none;">';
	//타이틀 노출
	studyModals += '<div class="titleArea">';
