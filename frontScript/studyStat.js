...
function listAct(){
	...
	actionArea += '<select name="contentSel" onchange="listSelect()">';
    actionArea += '</select>';
...
function excelAct() {
	...
	top.location.href='periodStatsTotal.php?'+searchValue;
}
...
function ajaxAct(sortData){
		...
		i = data.totalCount - (page - 1) * listCount; //게시글 번호 역순
