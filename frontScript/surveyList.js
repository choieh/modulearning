...
function listAct(page){
	...
	actionArea += '<button class="fRight" type="button" onClick="searchAct()" style="margin-left:10px">검색하기</button>';
  ...
  actionArea += '</select>';
  actionArea += '</div>'
  actionArea += '<span>설문지</span>'
  actionArea += '<select id="survey" name="surveySetSeq">'
  actionArea += '</select>'
  actionArea += '<span style="font-weight:bold; "> * 통계와 엑셀은 설문지 항목포함 모든 항목이 필수입니다.</span>';
  actionArea += '<button class="fRight" type="button" onClick="excelAct()" style="margin-left:10px">엑셀 다운로드</button>';
  actionArea += '<button class="fRight" type="button" onClick="statisTics()" style="margin-left:10px">통계내기</button>';
	actionArea += '</form></div>';
	...
	contents += '<tr><td class="notResult" colspan="5">검색값을 선택하세요.</td></tr>'	;
...
function ajaxAct(sortDatas){
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+sortData,function(data){
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
			...
			lists += '<tr><td class="notResult" colspan="5">조회 권한이 없습니다.</td></tr>'
		}else{
			lists += '<tr><td class="notResult" colspan="5">검색 결과가 없습니다.</td></tr>'
		}
...
function searchTypeSelect(types){
	$('.searchArea .searchChangeArea select, .searchArea input[type="text"], .searchArea strong').remove();
	var chageSearch ='';
	if(types == 'searchDate'){
...
function searchSurvey(){
  fetch("https://modulearning.kr/api/apiSurveySet.php?page=1&list=99")
  .then(res=>res.json())
  .then(data=>{
    survey.innerHTML += '<option value="">설문지를 선택하세요</option>'
    if( data.totalCount > 0){
      data.surveySet.map(v=> survey.innerHTML += `<option value="${v.seq}">${v.surveySet}</option>`)    
    }
  })
...
function excelAct() {
	...
	// top.location.href='surveyExcelDown.php?'+searchValue;
  let formDom = document.querySelector('#contents > div.searchArea > form'),
  formData = new FormData( formDom ), 
  params

  if( !formData.get('lectureDay') ) {
    alert('기간을 선택하세요')
    return
  }

  if( !formData.get('contentsCode') ) {
    alert('과정을 선택하세요')
    return
  }

  if( !formData.get('surveySetSeq') ) {
    alert('설문지를 선택하세요')
    return
  }
  
  params = `lectureDay=${ formData.get('lectureDay') }&contentsCode=${ formData.get('contentsCode') }&surveySetSeq=${ formData.get('surveySetSeq') }`

  window.open(`/admin/surveyExcelDown.php?${params}`)
}
...
function statisTics(){
  ...
  window.open(`/admin/statistics.php?${params}`)
