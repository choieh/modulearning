...
function listAct(page){
  ...
  actionArea += '</select>';
	...
	if(loginUserLevel < '5') {
		contents += '<th style="width:80px;">재전송</th>';
	}
	...
	var listAjax = $.get(useApi,'page='+page+'&list='+listCount+'&serviceType='+serviceType+sortData,function(data){
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
			$.each(data.study,  function(){
				...
				if(loginUserLevel < '5') {
					lists += '<td>';
					lists += '<button type="button" onClick="lmsJoinButton(\''+this.user.userID+'\',\''+this.lectureOpenSeq+'\',\''+this.contents.contentsCode+'\')">재전송</button>';
					lists += '</td>';
				}
				lists += '</tr>';
				j--;
...
function lmsJoinButton(userID,lectureOpenSeq,contentsCode){
	//수강연동 평가데이터 전송	
