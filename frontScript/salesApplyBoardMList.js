        ...
        `;
            ...
            lists += `
                ...
                <div class="px-4 pb-4" onclick="viewAct('${c.seq}')">
                    ...
                    <span class="font-bold">사업주명:</span> ${c.company}</div>
                    <div class="border-b border-gray-200 w-full py-1">
                    <span class="font-bold">과정명:</span> ${c.subject}</div>
                    <div class="border-b border-gray-200 w-full py-1">
                    <span class="font-bold">수강기간:</span> ${c.addItem01} ~ ${c.addItem02}</div>
                </div>
...
async function getContents() {
    const result = await fetch(
        useApi + `?boardCode=16${order}&page=${page}${statusParam}${param}`, {
        method: 'GET'
    })
