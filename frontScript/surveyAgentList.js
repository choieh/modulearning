...
function listAct(page){
	...
	contents += '<th style="width:120px;">SURVEY_CATE_NAME</th>';
	contents += '<th style="width:200px;">SURVEY_TITLE</th>';
	contents += '<th style="width:120px;">SURVEY_TARGET</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+'&joinURL='+joinURL+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
					...
					lists += '<td>'+this.SURVEY_CATE_NAME+'</td>';
					lists += '<td>'+this.SURVEY_TITLE+'</td>';
					lists += '<td>';
					if(this.SURVEY_TARGET == '' || this.SURVEY_TARGET == null){
						lists += '-'
					}else{
						lists += this.SURVEY_TARGET;;
					}
					lists += '</td>';
