...
function writeAct(){
	...
	function writePrint(){
		var writes ='';
			...
			writes += '<input name="userName" type="text" /> <button type="button" name="userIDbtn" onClick="searchSelect(\'userID\',\''+memberApi+'\',\'\',\'5\')">검색</button>';
			writes += '</li>';

			//사업자
			writes += '<li id="matchingValue">';
			writes += '<h1>영업담당자 (하위)</h1>';
			writes += '<input name="userName" type="text" /> <button type="button" name="matchingValueBtn" onClick="searchSelect(\'matchingValue\',\''+memberApi+'\',\'\',\'6\')">검색</button>';
			writes += '</li>';	
