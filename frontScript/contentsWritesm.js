...
/*
			...
			*/
			...
			writes += '<li>'
			writes += '<h1>SSO코드</h1>';
			writes += '<input type="text" name="ssoCode" class="name" value="'+ssoCode+'" /> ※ 원에듀 LMS 수강연동 시 전달받은 과정코드를 입력합니다.';
			writes += '</li>'
		...
		if(writeType == 'tracse'){
			...
			writes += '<div class="halfDiv">';
			writes += '<h1>HRD 코드</h1>';
			writes += '<select id="hrdCodeSelect" name="hrdCodeSelect"></select>';
			writes += '</div>';
			writes += '<div class="halfDiv">';
			writes += '<h1>개설용도</h1>';
			...
			contents += '<th style="width:20%;">HRD코드</th>';
			contents += '<th style="width:25%;">개설용도</th>';
			contents += '<th style="width:20%;">수강일</th>';
			contents += '<th style="width:10%;">개설차수</th>';
			contents += '<th style="width:15%;">수정 / 삭제</th>';
			contents += '</tr></thead><tbody></div>';
	 ...
	 $.get(hrdCodeApi,{'contentsCode':contentsCode},function(data){
	  ...
	  $.each(data.contentsHrdCode, function(){
		if(this.hrdCode==null || this.hrdCode==''){
			...
			hrdCodesub = this.hrdCode;
		}
		cHrdCode += '<option value="'+this.hrdCode+'">'+hrdCodesub+'</option>';
	  });
	  $('#hrdCodeSelect').html(cHrdCode);
	});
}
	...
	$.get(hrdCodeApi,{'hrdCode':hrdCode,'serviceType':serviceType, 'contentsCode':contentsCode},function(data){
		...
		if(totalCount!=0){
			$.each(data.lectureHrd, function(){
					...
					lists += '<td><input type="text" id="hrdCode'+this.seq+'" value="'+this.hrdCode+'"></td>';
...
function hrdCodeUpdate(seq){
	var hrdCode = $("hrdCode"+seq).val().trim();
	...
	if(confirm('개설차수를 수정하시겠습니까?')) {
		$.ajax({
			url:hrdCodeApi,
			type:'POST',
			data:'seq='+seq+'&hrdCode='+hrdCode+'&openChapter='+openChapter,
			dataType:'text',
			...
			$.each(data.report, function(){
				if(action == 'copy'){					
					...
					if(this.referenceAttach != null){
						var files = '<div id="referenceAttach" class="attachFile"><a href="fileDownLoad.php?fileName='+this.referenceAttach+'&link='+this.referenceAttachLink+'" target="_blank">'+this.referenceAttach+'</a><button type="button" onclick="deleteFileAct(\'referenceAttach\')">첨부파일삭제</button></div><input type="checkbox" name="delFile04" value="Y" />';
						$('.writeform label.AttachFiles:eq('+filePoint+')').after(files);
						$('.writeform label.AttachFiles:eq('+filePoint+')').remove();
					}else{
						filePoint ++;
