...
//		actionArea += '<option value="80">수강연동(비환급)</option>';
		if(loginUserLevel <= 3) {
			actionArea += '<option value="8">수강연동</option>';
		}
		actionArea += '</select>';
	}
  ...
  actionArea += '<input type="text" style="width:100px;" name="contentsCode">';
	...
	if(loginUserLevel <= 8){
		if(loginUserLevel <= 3){
			...
			actionArea += ' <button type="button" onClick="ssoExcel()" style="margin-left:10px">안전기술교육원 수강연동</button>';
		}else{
		...
		if(loginUserID == 'khl' || loginUserID == 'ksa'){
			actionArea += ' <button style="float:right;" type="button" onClick="studyAct()" >수료증페이지 이동</button>';
			actionArea += ' <div style="clear:both"></div>';
		}
	}
	actionArea += '</div>';
	/*actionArea += '<div>';
			...
			$.each(data.study,  function(){
				...
				if(this.serviceType == '3' || (this.serviceType == '8' && this.price==0)) { // 비환급인 경우 평가없음
					...
					if(this.midStatus == 'Y') { // 응시
						...
						if(loginUserLevel == '8'){
							if(this.studyEnd == 'Y'){
								midStatus = this.midScore+"("+this.countMidScore+")"+'<br />'+this.midSaveTime;
							}else {
								midStatus = this.midScore+"("+this.countMidScore+")"+'<br />'+this.midSaveTime;
							}
				...
				if(loginUserLevel == '8' && this.resultView != 'Y') { //2018-01-23 강혜림 수정
					...
					lists += '<td>'+midStatus+'</td>';
				} else if(loginUserLevel == '5' || loginUserLevel == '6' || loginUserLevel == '8'){
					lists += '<td>'+midStatus+'</td>';
				} else {
				...
				if(this.serviceType == '3') { // 비환급인 경우 평가없음
					...
					if(this.testStatus == 'N') { // 미응시
						...
						if(loginUserLevel == '8'){
							if(this.studyEnd == 'Y'){
								testStatus = this.testScore+"("+this.countTestScore+")"+'<br />'+this.testSaveTime;
							}else {
								testStatus = this.testScore+"("+this.countTestScore+")"+'<br />'+this.testSaveTime;
							}
						}else if(loginUserLevel == '6' || loginUserLevel == '5'){
				...
				if(this.serviceType == '3' || (this.serviceType == '8' && this.price==0)) { // 비환급인 경우 평가없음
					...
					if(this.reportStatus == null) { // 과제 없는 과정
						...
						if(data.nowTime >= reportEndTime) {
							reportStatus = '<strong class="blue">채점 대기중</strong><br />(시간 초과로 인한 자동제출)'+reportTutorTempSave+'<br />'+this.reportSaveTime;
						} else {
						...
						if(loginUserLevel == '8'){
							if(this.studyEnd == 'Y'){
								reportStatus = this.reportScore+"("+this.countReportScore+")"+'<br />'+this.reportSaveTime;
							}else {
								reportStatus = this.reportScore+"("+this.countReportScore+")"+'<br />'+this.reportSaveTime;
							}
						}else if(loginUserLevel == '6' || loginUserLevel == '5'){
