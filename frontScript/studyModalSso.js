...
function agreeModal(types,contentsCode,lectureOpenSeq){
	...
	var cautionApi = $.get('../api/apiSiteInfo.php',{},function(data){
		if(types == 'mid'){
			...
			studyModals += '<p><p>1. 최종평가는 <strong>응시 제한시간</strong>이 있으며, <strong>평가지 상단에 잔여시간이 표시</strong>됩니다.&nbsp;</p>';
			studyModals += '<p>2. 시험 입장 시 제한시간은 <strong>접속 종료 등의 상태에서도 중단 없이 계속 흘러가게 됩니다</strong>.&nbsp;<br>';
			studyModals += '예) 시험 제한시간이 1시간인 경우, 5시에 입장하여 시험 도중 학습자의 사정 등으로 종료되더라도 6시에 자동 시험 종료&nbsp;</p>';
			studyModals += '<p>3. <strong>최종제출 버튼까지 클릭 후 제출</strong>해야지만 평가 응시가 완료됩니다.&nbsp;</p>';
			studyModals += '<p>4. 최종제출을 하지 않은 상태로 평가 시간이 경과한 경우에는 <strong>작성한 답안으로 임시 저장 후 자동 제출</strong>됩니다.</p>';
			studyModals += '<p>5. <strong>최종평가의 점수는 최종 수료 점수에 반영</strong>됩니다. 수료기준의 반영율을 확인하여 주십시오.&nbsp;&nbsp;</p>';
