...
function listAct(page) {
    ...
    actionArea += '<option value="5">산업안전5</option>';
    ...
    actionArea += '<option value="10">비환급(평가있음)10</option>';
    actionArea += '<option value="11">오프라인11</option>';
    actionArea += '</select>';
    actionArea += '</div>';
    ...
    actionArea += '<button type="button" style="margin-left:30px;" onclick="studyDateChange();">변경하기</button>';
...
function studyDateChange(type) {
    ...
    if (lectureDayBe) {
        //기간변경,교육비변경,문자수신변경을 제외한 나머지는 사업주 필수 선택
        if (!(workType == 'lectureDay') && !(workType == 'price') && !(workType == 'sms')) {
            if (!companyCodeBe) {
                alert('사업주를 선택해주세요');
                return;
            } else if (!contentsCodeBe) {
                alert('과정을 선택해주세요');
                return;
            }
        }
