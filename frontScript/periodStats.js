...
function listAct(page) {
	...
	actionArea += '<option value="0">환급여부</option>';
  ...
  actionArea += '</div>';
	...
	actionArea += '영업자 아이디 <input type="text" name="marketer" style="width:100px;"/>&nbsp;&nbsp;';
	...
	contents += '<th colspan="5">비환급과정(산업안전)</th>';
...
function ajaxAct(listPage,sortData) {
	...
	var contentsName
	if( page.match(/autoContentsCode=[0-9A-Z]{5}/) ){
		contentsName = $('select[name="autoContentsCode"] option:selected').text();
	} else {
		contentsName = '';
	}
	var listAjax = $.get(useApi,'page='+page+'&list='+listCount+sortData,function(data) {
		...
		if (totalCount != 0 && loginUserLevel <= userLevel) {
			...
			$.each(data.periodStats, function() {
				...
				lists += contentsName != '' ? `<td>${contentsName}</td>`:`<td> - </td>`;						//과정명
				...
				//비환급
				lists += '<td>'+toPriceNum(this.totalStudy2)+'('+toPriceNum(this.totalStudy3)+')'+'</td>'; 		//일반 비환급 수강인원(산업안전 비환급)
				lists += '<td>'+toPriceNum(this.totalPassOk2)+'('+toPriceNum(this.totalPassOk3)+')'+'</td>'; 		//일반 비환급 수료인원(산업안전 비환급)
				lists += '<td>'+toPriceNum(this.totalPassNo2)+'('+toPriceNum(this.totalPassNo3)+')'+'</td>'; 		//일반 비환급 미수료인원(산업안전 비환급)
				lists += '<td>'+Math.round(this.percent2)+'('+toPriceNum(this.percent3)+')'+'%</td>'; 		//일반 비환급 수료율(산업안전 비환급)
				lists += '<td>'+toPriceNum(this.totalPrice2)+'('+toPriceNum(this.totalPrice3)+')'+'</td>'; 		//일반 비환급 교육비(산업안전 비환급)

				totalStudyAll  = eval(this.totalStudy1)  + eval(this.totalStudy2) + eval(this.totalStudy3);
				totalPassOkAll = eval(this.totalPassOk1) + eval(this.totalPassOk2) + eval(this.totalPassOk3);
				totalPassNoAll = eval(this.totalPassNo1) + eval(this.totalPassNo2) + eval(this.totalPassNo3);
				totalPriceAll  = eval(this.totalPrice1)  + eval(this.totalPrice2) + eval(this.totalPrice3);
...
function searchTypeSelect(types) {
	...
	var chageSearch =''
	chageSearch += '<select name="serviceType" onchange="searchAct()">';
	chageSearch += '<option value="0">환급여부</option>';
	chageSearch += '<option value="1">환급</option>';
	chageSearch += '<option value="3">비환급</option>';
	chageSearch += '</select>';
	if (types == 'searchDate' || types == 'searchMonth') {
		...
		chageSearch += '</select>';
		if (types == 'searchDate') {
			chageSearch += '<select name="searchMonth" onchange="	(\'lectureDay\')">';var h= '';
...
function excelAct() {
	var contentsName = $('select[name="autoContentsCode"] option:selected').text()

	searchValue = $('.searchForm').serialize();
	if(contentsName != ''){
		searchValue = searchValue+'&contentsName='+contentsName;
	}
