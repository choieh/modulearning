	...
	if ($studyBlock.find('form').length != 0){ //스터디블록 닫기
		...
		var studyDetails = $.get(chapterApi,'contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq,function(data){
			...
			*/
			...
			//if((data.serviceType == '1' && data.certPass == 'N' && data.contentsAccredit < '2018-04-30') || (data.serviceType == '1' && today != certDate && data.contentsAccredit >= '2018-04-30')){
...
function studyPop(obj,chapter,types,captchChk){	
	...
	$.get(chapterApi,{'contentsCode':checkCode,'lectureOpenSeq':checkOpenSeq},function(data){
		...
		}else if(chapter != 1 && eval(data.progress[(chapter-2)].progress) <= 79 && data.midStatus == 'N'){
	...
	$.get(chapterApi,{'contentsCode':checkCode,'lectureOpenSeq':checkOpenSeq},function(data){
		...
		if(types == 'mid'){
			...
			if(eval(data.totalProgress) <= 79){
				if(types == 'final'){
					alert('진도율 80% 이상 응시 가능합니다.')
				}else{
					alert('진도율 80% 이상 제출 가능합니다.')
				}				
			}else if(types == 'final'){
...
function progressChange(checkSeq,checkCode,checkOpenSeq,checkchapter){
	var check= $.get(chapterApi,'contentsCode='+checkCode+'&lectureOpenSeq='+checkOpenSeq,function(data){
		...
		//진도율에 따른 평가상태 변경
		if(($beforeProgress <= eval(data.midTestProgress) && eval(data.totalProgress) >= eval(data.midTestProgress))||($beforeProgress <= 79 && data.totalProgress >= 80)){
...
function chkPassTable(data,writeTarget){//수강기준 테이블
	...
	if(data.totalPassReport != 0){
		...
		chkTable += '<td colspan="3" style="padding:5px 0 ">반영된 평가, 과제 점수 합산 <strong>'+data.passScore+'</strong>점 이상 (단, 최종평가 '+data.passTest+'점, 과제 '+data.passReport+'점 미만 시 과락 적용)</td>';
	} else {
		...
		chkTable += '<td colspan="3" style="padding:5px 0 ">반영된 평가 점수 합산  <strong>'+data.passScore+'</strong>점 이상 (단, 최종평가 '+data.passTest+'점 미만 시 과락 적용)</td>';
	}
	chkTable += '</tr>';
...
function chkTestArea(data,writeTarget){//평가진행 관련 버튼ul
	...
	if(data.totalProgress <= 79 && data.testStatus != null){
		fintext = '진도율 80% 이상 응시 가능'
		finMsg = '<strong class="red">진도 부족</strong>';
		finLink = 'alert(\'진도율 80% 이상 응시 가능합니다.\')';
	...
	if(data.totalProgress <= 79 && data.reportStatus != null){
		reporttext = '진도율 80% 이상 제출 가능'
		reportMsg = '<strong class="red">진도 부족</strong>';
		reportLink = 'alert(\'진도율 80% 이상 제출 가능합니다.\')';
				...
				if(midTerm != 0){
					...
					if(eval(data.totalProgress) >= eval(data.midTestProgress)){
						if(data.midStatus == 'Y' || data.midStatus == 'C'){ //채점전
							...
							studyList += '<td class="midStatus"><strong class="red">응시 중<br>'+data.lectureEnd.substr(0,10)+' 23시 50분까지</strong></td>';
						}else{
