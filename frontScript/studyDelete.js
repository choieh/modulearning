...
function listAct(page){
    ...
    actionArea += '</select>';
	actionArea += '&nbsp;&nbsp;&nbsp;&nbsp;<button style="float:middle" type="button" onClick="allDelete()">전체 삭제 실행</button>';
	actionArea += '&nbsp;&nbsp;&nbsp;&nbsp;<span>이름/ID</span>';
	actionArea += '<input type="text" name="searchUser">';
...
function allDelete(){
	...
	if ($('select[name=lectureDay] option:selected').val() == '') {
		alert('기간을 선택해야 전체 삭제가 가능 합니다.');
		return;
	}
	if ($('select[name=companyCode] option:selected').val() == '') {
		alert('사업주를 선택해야 전체 삭제가 가능 합니다.');
		return;
	}
