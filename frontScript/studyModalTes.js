	...
	if($('input[name="agree"]:checked').prop('checked') == true){
		...
		if(types == 'final') {
			$.get(chapterApi,{'contentsCode':contentsCode,'lectureOpenSeq':lectureOpenSeq},function(data){
				...
				min = tday.getMinutes();  
				sec = tday.getSeconds();
				finalTime = year + '년 ' + month + '월 ' + day + '일 ' + hour +'시 ' + min + '분'; 

				//if(confirm('응시 제한시간이 있으며, 제한시간 내에 완료해야 합니다. \n응시 도중 접속종료를 하더라도 제한시간은 계속 흘러갑니다. \n지금 평가를 응시하시겠습니까?')){
				if(confirm('응시 제한시간 '+ data.testTime +'분이며, '+ finalTime +'까지\n완료해야 합니다. \n응시 도중 접속종료를 하더라도 제한시간은 계속 흘러갑니다. \n지금 평가를 응시하시겠습니까?')){
	...
	if(types=='mid' || types=='final'){ // 중간, 최종평가		
		var answerAjax = $.get(testApi,'testType='+types+'&contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq+'&list=99',function(data){			
			...
			if(types == 'final'){
				...
				//setClock();
				if(typeof setIntervalTime == 'number') clearInterval(setIntervalTime);
				setIntervalTime = setInterval(setClock, 1000);
...
function answerEnd(types,contentsCode,lectureOpenSeq,examList,examPage,testEnd) {
    ...
    if(types == 'mid' || types == 'final') {
        ...
        studyModal += '<button type="button" onclick="reportSave(\''+testEnd+'\'),studyRemove()">확인</button>'
    }
		...
		success:function(data){
			if(testEnd == 'Y') {
				if(data.result == 'success'){
                    ...
                    var studyModal = '';
                    studyModal += '<div class="studyModal">'
                    studyModal += '<h1>최종제출 되었습니다. 결과 확인은 학습종료일('+lectureEnd[1]+'월 '+lectureEnd[2]+'일) 이후 1주일 이내에 가능하며, 휴대전화로 점수확인 문자가 발송됩니다. </h1>'
					...
					studyModal += '<div class="dark"></div>'
                    studyModal += '<div class="studyModal">'
                    studyModal += '<h1>응시시간이 종료되었습니다. 마지막 저장된 답안까지 자동 제출됩니다.<br>결과 확인은 학습종료일('+lectureEnd[1]+'월 '+lectureEnd[2]+'일) 이후 1주일 이내에 가능하며,<br>휴대전화로 점수확인 문자가 발송됩니다. </h1>'
                    studyModal += '<button type="button" onclick="studyModalClose(); viewStudyDetail('+studySeq+',\''+contentsCode+'\','+lectureOpenSeq+');">확인</button>'
                    studyModal += '</div>'
                    $('#screenModal').append(studyModal)
					//listAct();
					...
					$.get(reportApi,{'contentsCode':contentsCode,'lectureOpenSeq':lectureOpenSeq},function(data){
						$.each(data.studyReport, function(){
							...
							if(this.answerType == "text"){
								...
								file1 = 'http://modulearning.kr'+this.attachLink;
							}
							alert("제출이 완료되었습니다.");
...
function studyModalClose(types) {
	if(types=='final'){
        ...
        var studyModal = '';
            ...
            studyModal += '<button type="button" onclick="answerSave(\''+types+'\',\''+contentsCode+'\',\''+lectureOpenSeq+'\',\''+examList+'\',\''+examPage+'\',\'\',\'\',\'Y\'),studyRemove(\'auto\')">확인</button>'
            studyModal += '<button type="button" onclick="answerRomeve()">취소</button>'
            studyModal += '</div>'
            $('#screenModal').append(studyModal)
	}else if(types=='mid'){
        ...
        var studyModal = '';
            ...
            studyModal += '<button type="button" onclick="studyRemove(\'auto\')">확인</button>'
            studyModal += '<button type="button" onclick="answerRomeve()">취소</button>'
            studyModal += '</div>'
            $('#screenModal').append(studyModal);
	}else{
...
function certPassModal(startDate,studySeq,contentsCode,lectureOpenSeq){
	...
	var cautionApi = $.get('../api/apiSiteInfo.php',function(data){
        ...
        studyModals += '<button type="button" class="btnIpin" onClick="window.open(\'../attach/docs/한국산업인재융합교육원_민간아이핀.pdf\')">민간아이핀 발급순서 보기</button></form>';
		studyModals += '<br /><span class="red ipinInfo">※ 타인 명의 휴대폰인 경우 민간아이핀을 발급 후 아이핀 인증으로 진행해 주시기 바랍니다. (아이핀/마이핀가입)</span>';
...
function mobileCheck(userName,userBirth,sex){
	if(userBirth.substring(2) != loginUserBirth) {
		alert('본인인증은 되었으나 한국산업인재융합교육원에 등록된 회원정보와 일치하지 않습니다. 문제가 있는 경우 고객센터로 연락주시기 바랍니다.');
	} else {
