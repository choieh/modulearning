	...
	var listAjax = $.get(useApi,'page='+page+'&list='+listCount+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.popup, function(){
				...
				lists += '<td class="left" onClick="openPopups(\''+this.subject+'\',\''+this.popupURL+'\',\'_'+this.popupTarget+'\',\''+this.attachFile+'\',\''+this.width+'\',\''+this.height+'\')" style="cursor:pointer;">'+this.subject+'</td>';
