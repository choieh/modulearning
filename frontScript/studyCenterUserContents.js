...
function listAct(page) {
    ...
    $.get(checkDateApi, {'companyID': companys}, function (data) {
        ...
        if (data.studyCenter && subDomain != 'educenter') {
...
function ajaxAct(listPage) {
    ...
    if (subDomain == 'educenter') {
        useApi = '../api/apiContentsUserEduCenter.php'
    }
    ...
    var listAjax = $.get(useApi, mapping + '&page=' + page + '&list=' + listCount + sortData + sort01s + sort02s, function (data) {
        ...
        if (totalCount != 0) {
            ...
            $.each(data.contents, function () {
                ...
                //버튼영역
                if (subDomain == 'educenter') {
                    lists += `<a href="${bookURL}/${this.bookImage}" target=""><button type="button"><img src="../images/lecture/img_button_detail.png" alt="상세보기이미지"><br />상세보기</button></a>`
                } else {
                ...
                if (startTime <= nowTime && endTime >= nowTime) {
                    ...
                    if (inputStudyType == 'ord') {
                        ...
                        if (subDomain == 'educenter') {
                            if (this.educenterApplyLimit == 'Y') {
                                lists += `<button type="button" onClick="alert('교육 신청 기간이 지났습니다.')">
                                <img src="../images/lecture/img_button_submit.png" alt="신청하기이미지"><br />신청하기</button>`;
                            } else {
                                // lists += `<button type="button" onClick="lectureSubmitEdu(
                                //     '${this.mpServiceType}', '${this.contentsCode}',
                                //     '${this.contentsName.replace(/"/g, '&quot;').replace(/'/g, '&quot;')}', ${this.price},
                                //     '${this.educenterLectureDay}', '${this.educenterLectureDayEnd}')">
                                // <img src="../images/lecture/img_button_submit.png" alt="신청하기이미지"><br />신청하기</button>`;

                                lists += `<button type="button" onClick="lectureSubmitEdu(
                                    {types: '${this.mpServiceType}', contentsCode: '${this.contentsCode}', 
                                    contentsName: '${this.contentsName.replace(/"/g, '&quot;').replace(/'/g, '&quot;')}', 
                                    price: ${this.price}, educenterLectureDay: '${this.educenterLectureDay}', 
                                    educenterLectureDayEnd: '${this.educenterLectureDayEnd}', sort01: '${this.sort01}'})">
                                <img src="../images/lecture/img_button_submit.png" alt="신청하기이미지"><br />신청하기</button>`;
                            }
                ...
                if (this.previewImage != null) {
                    if (subDomain == 'educenter') {
                        lists += '<img src="' + imageURL + this.previewImage + '" onclick="location.href=\''+bookURL+'/'+this.bookImage+'\'" alt="' + this.contentsName.replace(/"/g, '&quot;').replace(/'/g, '&quot;') + '" />';
                    } else {
                    ...
                    if (subDomain == 'educenter') {
                        lists += '<img src="../images/lecture/img_noimage.png" alt="이미지준비중" />';
                    } else {
                ...
                if (subDomain == 'boram') {
                    lists += '<h3></h3>';
                } else if (subDomain == 'educenter') {
                    lists += `<h3><span style="font-weight: bold; color:ghostwhite; border: 1px solid forestgreen; background-color: #00adee; padding: 2px;">`
                    if (this.sort01 == 'lecture03') {
                        lists += '오프라인'
                    } else {
                        lists += '줌/오프라인'
                    }
                    lists += '</span>'
                    lists += `${this.sort01Name} >  ${this.sort02Name}`;
                    lists += '</h3>'
                } else {
                ...
                if (subDomain == 'educenter') {
                    lists += '<h1 onclick="location.href=\''+bookURL+'/'+this.bookImage+'\'">' + this.contentsName + '</h1>'
                } else {
                ...
                if (subDomain == 'educenter') {
                    lists += `<span style="font-size: 13px;">`;
                    lists += `<b style="color: #5758BB">교육 일자: ${this.educenterLectureDay}</b>`;
                    if (this.sort01 != 'lecture03') {
                        lists += `, <b style="color: tomato;"><a href="${this.intro}" target="_blank">과정소개 영상보기</a></b>`
                    }
                    lists += `</span><br>`;
                }
                ...
                if ((this.sort01 == 'lecture12' || this.sort01 == 'lecture03') && subDomain != 'educenter') {
    ...
    var listAjax = $.get(loginApi, function (data) {
        ...
        let setDatePicker = {
            ...
            beforeShowDay: function (date) {
                ...
                if (subDomain === 'iic' || subDomain === 'ourhome' || subDomain === '1078839288') {
                    return [true];
                }
...
function lectureSubmitHrd(types, contentsCode, contentsName, price, lectureStart, lectureEnd, inputStudyType) {
    ...
    var listAjax = $.get(loginApi, function (data) {
        ...
        if (types == 2) {
            modalWrite += '<li>';
            modalWrite += '<input type="hidden" name="contentsType" value="2">'
...
async function lectureSubmitEdu(conInfo) {
    // types, contentsCode, contentsName, price, educenterLectureDay, educenterLectureDayEnd
    // console.log(conInfo)
    // return;
    var url = window.location.href;
    var companyID = url.split('/');
    ...
    if (loginUserID == '') {
        if (confirm('로그인이 필요한 서비스입니다.\n로그인 하시겠습니까?') == true) {
            if (pageMode == 'studyCenterPage') {
                ...
                top.location.href = 'login.php?page=lecture.php'
    ...
    fetch(`/api/apiEduCenterOrder.php?contentsCode=${conInfo.contentsCode}&lectureStart=${conInfo.educenterLectureDay}`)
        .then(res => res.json())
        .then(data => {
            if (data.message == 'sufficient') {
                alert('신청 인원이 다 찼습니다.')
                return;
            } else {
                var listAjax = $.get(loginApi, function (data) {
                    ...
                    var lectureStart = conInfo.educenterLectureDay
                    var lectureEnd = conInfo.educenterLectureDayEnd


                    modalWrite += `<form class="orderSubmitHRD" style="font-size:14px;" id="orderSubmitHRD" method="post" action="/payments/educenterPay.php">`;
                    modalWrite += '<div id="modal">';
                    modalWrite += '<div class="lectureSubmit" style="height:auto;">';
                    modalWrite += '<h1>교육신청하기<button type="button" onClick="modalClose()"><img src="../images/admin/btn_close.png" alt="닫기" /></button></h1>';
                    modalWrite += '<div>';
                    modalWrite += '<div class="BBSList">';
                    modalWrite += '<input type="hidden" name="serviceType" value="' + conInfo.types + '">'; // 서비스
                    // 구분 : 1사업주, 2내일배움카드, 3비환급
                    modalWrite += '<input type="hidden" name="contentsCode" value="' + conInfo.contentsCode + '">';
                    modalWrite += '<input type="hidden" id="educationPrice" name="educationPrice" value="' + conInfo.price + '">';
                    modalWrite += '<input type="hidden" name="lectureStart" value="' + lectureStart + '">';
                    modalWrite += '<input type="hidden" name="lectureEnd" value="' + lectureEnd + '">';
                    modalWrite += '<input type="hidden" name="inputStudyType" id="inputStudyType" value="' + inputStudyType + '"/>';
                    ...
                    modalWrite += '<th style="width:70px;">구분</th>';
                    modalWrite += '</tr></thead><tbody>';
                    modalWrite += '<td>' + conInfo.contentsName + '</td>';
                    if (conInfo.types == 1) {
                        serviceTypeGubun = '환급';
                    } else if (conInfo.types == 2) {
                        serviceTypeGubun = '내일배움카드';
                    } else if (conInfo.types == 3) {
                        serviceTypeGubun = '비환급';
                    } else if (conInfo.types == 11) {
                        serviceTypeGubun = '오프라인';
                    }
                    modalWrite += '<td>' + serviceTypeGubun + '</td>';
                    modalWrite += '</tbody></table>';
                    modalWrite += '</div>';
                    ...
                    modalWrite += '<li>';
                    modalWrite += '<h1>휴대전화</h1>';
                    modalWrite += `<select name="recipientMobile01" class="${recipientMobile01}" style="appearance: auto; -webkit-appearance: auto">`;
                    if (loginUserID != '') {
                        modalWrite += '<option value="' + recipientMobile01 + '">' + recipientMobile01 + '</option>';
                    } else {
                        modalWrite += '<option value="010" >010</option>';
                    ...
                    modalWrite += '<div style="display: inline;" id="price">' + toPriceNum(conInfo.price) + '원</div>';
                    modalWrite += '</li>';

                    if (conInfo.sort01 != 'lecture03') {
                        modalWrite += '<li>';
                        modalWrite += '<h1>수강 방법</h1>';
                        modalWrite += '<select name="educationType" style="appearance: auto; -webkit-appearance: auto;">';
                        modalWrite += '<option value="">선택</option>'
                        modalWrite += '<option value="줌">줌(온라인)</option>'
                        modalWrite += '<option value="집체">집체(오프라인)</option>'
                        modalWrite += '</select>';
                        modalWrite += '</li>';
                    } else {
                        modalWrite += `<input name="educationType" type="hidden" value="집체">`;
                    }


                    modalWrite += '<li>';
                    modalWrite += '<h1>환불 규정 확인</h1>'
                    modalWrite += `<button type="button" onclick="goRefundRegulation(this)" style="margin-left: 5px;">규정확인</button>`;
                    modalWrite += '</li>';
...
function goRefundRegulation(btn) {
    const existEle = btn.parentElement.querySelector('button')
    if (!btn.parentElement.querySelector('input[type="checkbox"]')) {
        const newEle = document.createElement('input');
        newEle.type = 'checkbox';
        newEle.name = 'refundRegulation'
        newEle.value = 'Y';
        newEle.checked = true;
        btn.parentElement.insertBefore(newEle, existEle);
    }
    window.open('https://educenter.modulearning.kr/studyCenterEC/bbs.php?boardCode=206&seq=10282', '_blank');
}
...
function viewAct(viewSeq, contentsCode) {
    ...
    $.get(useApi, {'seq': seq, 'contentsCode': contentsCode}, function (data) {
        ...
        $.each(data.contents, function () {
            if (sort01 == '') {
                if (domain == 'ourhome') {
                    $('#titleArea > h2 strong, #titleArea > h1').html('[신규입사자] Oh!Starter 과정');
                } else {
