...
function pageAct(){	
	...
	$.get(useApi,{'companyID':loginCompanyID },function(data){
		$.each(data.company, function(){
			...
			writes += '<input type="hidden" name="companyID" value="'+this.companyID+'"/>';
			//입력영역 시작
			writes += '<ul>';
			...
			if(this.companyCode == '2728101049'){
				writes += '<li><h1>교육과정리스트<br />양식 업로드</h1>';
				writes += '<input type="file" name="eduInfoList" class="subject" /> * 엑셀 파일로 업로드하시기 바랍니다.(파일명은 무관)';
				writes += '</li>';
			}
