...
function writeAct(writeSeq, Types, testType){
	...
	if(seq != ''){
		if(writeType=='contentsWrite'){
			...
			writes += '<h1>HRD 코드</h1>';
			writes += '<select id="hrdCode" onchange="categoryChange(this.value)"><option value="popular">인기</option><option value="new">신규</option></select>';		
			writes += '</div>';
			writes += '<div class="halfDiv">';
			writes += '<h1>개설용도</h1>';
			writes += '<select id="orderBy_popular" style="display:;"><option value="1">사업주지원(환급)</option><option value="3">비환급(평가없음)</option><option value="5">비환급(평가있음)</option></select>';
			writes += '</div>';
			writes += '</li>';
			writes += '</ul>';
