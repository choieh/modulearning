			...
			if (data == 'error'){
				...
				if(confirm('로그인에 실패하였습니다.\n\n아이디 또는 비밀번호를 확인 바랍니다.\n\n비밀번호 찾기로 이동하시겠습니까?')){
					...
					if(hostname[0] == 'security'){
						top.location.href='../security/login_3.php?mode=findPW';
					}else{
						top.location.href='../member/login_3.php?mode=findPW';
					}
...
function loginPage (types,nextAct){
	...
	if(types == 'login'){
		if(pageMode == 'userPage'){
			...
			pageWrites += '<p style="color: #fff;font-size: 50px;font-weight: bold;">LOGIN</p>';
		...
		pageWrites += '<button type="submit" style="background-color: #333;"><img src="../images/member/btn_login.png" alt="로그인" />로그인</button>';
		pageWrites += '</form>';
		...
		if(pageMode != 'mobilePage'){
			if(pageMode == 'userPage'){
				...
				pageWrites += `| <a href="javascript:top.location.href='../${studyCenterLocate}/mypage.php'">회원가입</a>`;
			}else{
