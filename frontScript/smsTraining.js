...
var smsApi = '../api/apiSMStraining2.php';	
...
function listAct(page){
  ...
  contents += '<th style="width:150px;">영업 팀장 ID / 이름<br />이메일</th>';
    ...
    contents += '<th style="width:105px;"><input type="checkbox" id="sendReport" onChange="checkboxAllCheck(\'sendReport\')" /><label for="sendReport">교육실시계획서</label></th>';
    contents += '<th style="width:105px;"><input type="checkbox" id="sendCertificate" onChange="checkboxAllCheck(\'sendCertificate\')" /><label for="sendCertificate">수료증안내</label></th>';
	...
	contents += '<th style="width:105px;"><input type="checkbox" id="sendProgress" onChange="checkboxAllCheck(\'sendProgress\')" /><label for="sendProgress">학습진도현황</label></th>';
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
          ...
          if(this.header == undefined){
            ...
            lists += `<td>${this.header}(${this.headerID??'-'})<br>${(this.headerEmail01??'-')+'@'+this.headerEmail02??'-'}</td>`;
					...
					lists += '<td><input type="checkbox" id="sendReport'+this.seq+'" name="sendType[]" class="sendReport" value="report/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'/" /><label for="sendReport'+this.seq+'">선택하기</label><br />';
                    lists += '<button type="button" onclick="viewPop(\'report/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'\')">내용보기</button></td>';
                    lists += '<td><input type="checkbox" id="sendCertificate'+this.seq+'" name="sendType[]" class="sendCertificate" value="certificate/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'/" /><label for="sendCertificate'+this.seq+'">선택하기</label><br />';
					lists += '<button type="button" onclick="viewPop(\'certificate/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'\')">내용보기</button></td>';
					lists += '<td><input type="checkbox" id="sendContract'+this.seq+'" name="sendType[]" class="sendContract" value="contract/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'/" /><label for="sendContract'+this.seq+'">선택하기</label><br />';
					lists += '<button type="button" onclick="viewPop(\'contract/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'\')">내용보기</button></td>';
					lists += '<td><input type="checkbox" id="sendProgress'+this.seq+'" name="sendType[]" class="sendProgress" value="progress/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'/" /><label for="sendProgress'+this.seq+'">선택하기</label><br />';
					lists += '<button type="button" onclick="viewPop(\'progress/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'\')">내용보기</button></td>';
					...
					//lists += '<button type="button" onclick="viewPop(\'sms/'+this.userID+'/'+this.lectureStart+'/'+this.lectureEnd+'/'+this.companyCode+'\')">내용보기</button></td>';
					lists += '</td>';
					lists += '</tr>';
