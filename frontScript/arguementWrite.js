...
function writeAct(writeSeq,writeReply){
	...
	function writePrint(){
		var writes ='';
		if(addItem01.split('~') != '' || addItem01.split('~') != null){
			var startDate = addItem01.split('~');
		}
			...
			writes += '<ul>';
			writes += '<li class="title"><h1>토론주제</h1>';
			writes += '<input type="text" class="subject" name="subject" value="'+writeSubject+'"/>';
			writes += '<input type="hidden" name="titleSubject" value="Y">';
			writes += '</li>'
			writes += '<li class="picker"><h1>토론 기간</h1>';
			if(startDate[0] == '' || startDate[0] == null){
				writes += '<div class="datePicker"><input type="text" name="startDate" class="cal" value="" readonly="readonly" /></div>&nbsp;~&nbsp;';
				writes += '<div class="datePicker"><input type="text" name="endDate" class="cal"  value="" readonly="readonly" /></div>&nbsp; * 기간 입력 필수';
				writes += '<input type="hidden" name="addItem02" value="Y">'
			}else{
				writes += '<div class="datePicker"><input type="text" name="startDate" class="cal" value="'+startDate[0]+'" readonly="readonly" /></div>&nbsp;~&nbsp;';
				writes += '<div class="datePicker"><input type="text" name="endDate" class="cal"  value="'+startDate[1]+'" readonly="readonly" /></div>&nbsp; * 기간 입력 필수';
			}
			writes += '</li>';
			$('.title').after(writes);
			if(useCategory == 'Y'){
