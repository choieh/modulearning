...
var chapterApi = '../api/apiStudyChapterK.php';
...
//버튼 변환스크립트
function writeBtn(btnType, chapter, reportSeq) {
    ...
    if (btnType == 'study') { //학습하기 관련
        ...
        writes += '<button type="button" title="과제 제출" onclick="chkTest(\'report\',this)" value="'+reportSeq.seq+'">';
...
//  var captchaPage = 'captcha.php';
    ...
    $.get(chapterApi, {'contentsCode': checkCode, 'lectureOpenSeq': checkOpenSeq}, function (data) {
        ...
        } else if (Number(midTerm) != 0 && (Number(chapter) > Number(midTerm)) && (data.midStatus == 'N' && data.midStatus == 'V')) {
            alert('중간평가 응시 완료 후에 학습할 수 있습니다.');
        } else if (Number(midTerm) != 0 && (Number(chapter) > Number(midTerm)) && data.midStatus == 'Y') {
            alert('중간평가 채점 완료 후에 학습할 수 있습니다.');
...
function chkTest(types, obj) {
    ...
    if(serviceType == 6){
        localStorage.setItem('reportSeq', obj.value);
    }
...
//  var links = 'captcha.php'
    ...
    $.get(chapterApi, {'contentsCode': checkCode, 'lectureOpenSeq': checkOpenSeq, 'reportSeq':obj.value}, function (data) {
        ...
        if (types == 'mid') {
            ...
            // if (data.serviceType == 6) {
            //     data.finalTestProgress = 50;
            // }
...
//             studyList += '</td>'
            ...
            } else if (data.serviceType == 6) {
                // if (eval(midTerm) != 0 && (eval(data.progress[i].chapter) > eval(midTerm)) && (useReport == 'N' || useReport == 'V')) {
                //     studyList += writeBtn('study', '중간평가 응시 완료 후에 학습할 수 있습니다.');
                // } else if (eval(midTerm) != 0 && (eval(data.progress[i].chapter) > eval(midTerm)) && useReport == 'Y') {
                //     studyList += writeBtn('study', '중간평가 채점 완료 후에 학습할 수 있습니다.');
                // } else {
                //     studyList += writeBtn('study', (i + 1));
                // }
                studyList += writeBtn('study', (i + 1));
            ...
            studyList += '</tr>';
        } else if (data.serviceType == 1 || data.serviceType == 9 || data.serviceType == 5 || data.serviceType == 6) { //2017.07.19 강혜림 서비스타입 5번 추가
            //중간평가 노출
            ...
            } else if (useReport != null && data.serviceType == 6) {
                if (midTerm != 0) {
                    studyList += '<tr class="midtestLine" style="background-color:#E7F6FF;">';
                    studyList += '<td>[평가]</td>';
                    studyList += '<th class="blue"><strong>중간평가</strong>';
                    ...
                    if (data.reportStatus == 'N' || data.reportStatus == 'V') {
                        ...
                        studyList += '<br />평가 응시시간 : ' + data.reportSaveTime + '<br />';
                        studyList += '접속 아이피 : ' + data.reportIP + '</th>';
                    }
                    //응시상태
                    if (eval(data.totalProgress) >= eval(data.midTestProgress)) {
                        if (data.reportStatus == 'Y' || data.reportStatus == 'C') { //채점전
                            studyList += '<td class="midStatus"><strong class="blue">응시 완료</strong></td>';
    ...
    if(data.serviceType == 6){
        for(let x = 0; x < data.reportEA; x++){
           studyList += reportForKDG(data, data.report[x])
        }
    }
...
function openStudyDiv(className) { // 수강창
    ...
    modalWrite += `<span style="display:block; background:#f2f6f7; text-align:right; padding:3px; cursor:pointer;" onClick="studyEnd('${className}')">
    ...
    modalWrite += `<iframe id="frame" name="frame" src="${contentsInfo.contentsUrl}" style="width:inherit; height:inherit;" allowfullscreen></iframe>`;
...
function studyModal(obj, chapter) { // 모달 창 오픈
    var runningForm = '';
    if (typeof (obj) == 'object') {
        runningForm = $(obj).parents('form');
    } else {
    ...
    contentsInfo = getContentsInfo(checkCode, checkOpenSeq, chapter); // 콘텐츠 정보 가져옴

    if (Number(contentsInfo.prevProgress) != 100 && serviceType != 6) { // 이전 차시 진도율 100%
...
function reportForKDG(data,reportSeq){
    let row ='';
    row += '<tr class="reportLine" style="background-color:#E7F6FF;">';
    row += '<td>[과제]</td>';
    row += '<th class="blue"><strong>과제 제출</strong>';
    let time = '', timeover =false;
    
    
    if(reportSeq.insertDate != null){
        //응시 후 30분 작성가능 한 경우 사용
        time = (function (reportDate){
            let a = new Date(reportDate)
            a.setMinutes(a.getMinutes() + 30)
            return a
        })(reportSeq.insertDate);
    }

    if (data.reportStatus == 'N' || data.reportStatus == 'V') {
        row += '</th>';
    } else {
        row += '<br />과제 제출 시간 : ' + (reportSeq.inputDate ?? '미제출') + '<br />';
        // row += '접속 아이피 : ' + data.reportIP + '</th>';
    }
    //메세지 출력
    if (data.totalProgress >= eval(data.finalTestProgress)) {

        if(time !=''){
            // timeover = time.getTime() <= new Date().getTime() ? true : false;
            timeover = new Date(data.lectureEnd).getTime() <= new Date().getTime() ? true : false;
        }

        if (data.reportStatus == 'Y' || data.reportStatus == 'C' || timeover) {
            row += '<td class="reportStatus"><strong class="blue">응시 완료</strong></td>';
        } else if (reportSeq.insertDate != null) {
            // time = `${time.getFullYear()}-${time.getMonth()+1}-${time.getDate()} ${time.getHours()}:${time.getMinutes()}`;
            // row += '<td class="reportStatus"><strong class="red">작성 중<br>' + time + ' 까지</strong></td>';
            row += '<td class="reportStatus"><strong class="red">작성 중<br>' + data.lectureEnd.substr(0, 10) + ' 23:50까지 </strong></td>';
        } else {
            row += '<td class="reportStatus"><strong class="blue">응시 가능</strong></td>';
        }

    } else {
        row += '<td class="reportStatus"><strong class="red">진도 부족</strong></td>';
    }
    ...
    // if (data.reportStatus == 'Y' || data.reportStatus == 'C') {
    if (0) {
        row += writeBtn('reportComplete');
    } else {
        row += writeBtn('reportWrite','',reportSeq);
    }
