function listAct(page){
    ...
    actionArea += '&nbsp;&nbsp;<input type="radio" name="messageType" value="SMS" id="SMS"><label for="SMS">단문전송실패</label>';
    actionArea += '<input type="radio" name="messageType" value="MMS" id="MMS"><label for="MMS" style="margin-right:20px;">장문전송실패</label>';	
...
function ajaxAct(sortDatas){
    ...
    $.get('../api/apiKakaoLog.php','&page='+page+'&list='+listCount+sortData,function(data){
        var lists = '';
        ...
        if(totalCount != 0){ 
            $.each(data.sendLog, function(){
                ...
                //lists += '<td><input type="checkbox" id="resendChk"><label for="resendChk">준비중</label></td>';
                lists += '</tr>';
                i--;
            })
        }else{
