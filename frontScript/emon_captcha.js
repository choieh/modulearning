...
var useApi = '../api/apiEmonCaptcha.php';
...
function listAct(page){
	...
	actionArea += '</div>';
	actionArea += '<strong style="color:red">* 캡챠 인증관련 데이터는 재전송이 불가합니다.</strong>';
	actionArea += '</div>';
	actionArea += '</form>'
	actionArea += '</div>';
	...
	contents += '<th style="width:120px;">중간평가 캡챠 인증시간<br>응시IP</th>';
	contents += '<th style="width:120px;">최종평가 캡챠 인증시간<br>응시IP</th>';
	contents += '<th style="width:120px;">과제 캡챠 인증시간<br>응시IP</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&list='+listCount+sortData,function(data){
		...
		if(totalCount != 0){
			$.each(data.emon,  function(){
				lists += '<tr>';
				lists += '<td>'+i+'</td>';
				if(this.midCaptchaTime == '미응시' || this.midCaptchaTime == '-'){
					lists += '<td>'+this.midCaptchaTime+'<br>'+this.midIP+'</td>';
				}else{
					lists += '<td><strong style="color:red">'+this.midCaptchaTime+'</strong><br>'+this.midIP+'</td>';					
				}	
				
				if(this.testCaptchaTime == '미응시' || this.testCaptchaTime == '-'){
					lists += '<td>'+this.testCaptchaTime+'<br>'+this.testIP+'</td>';
				}else{
					lists += '<td><strong style="color:red">'+this.testCaptchaTime+'</strong><br>'+this.testIP+'</td>';					
				}

				if(this.reportCaptchaTime == '미응시' || this.reportCaptchaTime == '-' || this.reportCaptchaTime == '과제없음'){
					lists += '<td>'+this.reportCaptchaTime+'<br>'+this.reportIP+'</td>';
				}else{
					lists += '<td><strong style="color:red">'+this.reportCaptchaTime+'</strong><br>'+this.reportIP+'</td>';					
				}				
