...
function loginPage (types,nextAct){
	...
	if(types == 'login'){
      ...
      pageWrites += '<div>'
			...
			pageWrites += `<img style="margin-top: 20px; width:50px;" src="/attach/studyCenter/logo_${location.hostname.split('.')[0]}.png" alt="로고" />`;
      pageWrites += `<span style="
  ...
  ">사이버 연수원</span>`;
      pageWrites += '</div>'
		...
		pageWrites += '<button id="loginBtn" type="submit"><img src="../images/member/btn_login.png" alt="로그인" />로그인</button>';
		pageWrites += '</form>';
