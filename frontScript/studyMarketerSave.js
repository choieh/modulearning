...
function listAct(page){
    ...
    //영업자 배정
    actionArea += '<div>';
    actionArea += ' <a id="marketerID">';
	actionArea += '   <span style="width: 65px;">영업자 배정</span>';
    //actionArea += '   <input name="userName" type="text">';
    ...
    actionArea += ' </a>';
    actionArea += ' <button type="button" onclick="marketerSave();">영업자 배정 등록</button>';
	actionArea += '</div>';
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
			$.each(data.study,  function(){
				if(this.serviceType == '1') {
					...
					serviceType = '수강연동<br><span style="color:red;">(비환급)</span>';
				} else {
					serviceType = '테스트';
				}
				lists += '<tr>';
				...
				if(this.serviceType == '3') { // 비환급인 경우 진도율 80%이면 수료
					if(this.lectureStart <= today && this.lectureEnd >= today){
						...
						if(this.progress >= 80) { // 수료
							...
							passOK = '<strong class="blue" style="cursor:pointer;">수료</strong>';
						}else{
...
function marketerSave(){

    var lectureDayChk = $('select[name=lectureDay]').val();
    var companyCodeChk = $('select[name=companyCode]').val();
    var marketerIDChk = $('select[name=marketerID]').val();

    var valiMsg = '';

    if (lectureDayChk == undefined || lectureDayChk == '') valiMsg = '수강기간을 선택해주세요';
    else if (companyCodeChk == undefined || companyCodeChk == '') valiMsg = '사업주를 선택해주세요';
    else if (marketerIDChk == undefined || marketerIDChk == '') valiMsg = '영업자를 검색해서 선택해주세요';
    ...
    if(confirm('조회한 수강의 영업자를 배정하시겠습니까?')){
        $.post(apiMarketerSaveApi,{'lectureDay':lectureDayChk, 'companyCode':companyCodeChk, 'marketerID':marketerIDChk},function(data){

            console.log('marketerSave data',data);
            if(data.result = 'success'){
                alert('성공적으로 영업자가 배정되었습니다.');
                searchAct();
            }else{
                alert('등록을 실패하였습니다.');
            }
