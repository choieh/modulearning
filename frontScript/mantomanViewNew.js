...
function viewAct(writeSeq,addItem02){
	...
	viewAjax = $.get(useApi,{'type':type,'viewType':viewType,'seq':seq,'boardType':boardType,'addItem02':addItem02},function(data){
		...
		$.each(data.consult,function(){
			...
			writes += '<strong class="tel">'+this.email01+'@'+this.email02+'</strong>';
			...
			//if(this.addItem02 == 'landing'){
				...
				writes += '<td>직급/직책</td><td>'
				if(this.position != '' && this.position != null){
					writes += this.position+'</td></tr>';
				}else{
					writes += '</td></tr>';
				}
                writes += '<tr><td>연락처</td><td>'+this.phone01+'-'+this.phone02+'-'+this.phone03+'</td>';
                writes += '<td>이메일</td><td>'+this.email01+'@'+this.email02+'</td>';
                writes += '<td>방문경로</td><td>'
				if (this.visitPath in visitPathList) {
					writes += visitPathList[this.visitPath]+'</td>'
				} else {
					writes += this.visitPath+'</td>';
				}
            	writes += '</tr>';
				writes += '<tr><td>문의내용</td><td style="width:500px">'+this.content.replace(/\n/g,'<br />').replace(/  /g,'&nbsp;&nbsp;')+'</td></tr>';
				writes += '</tbody></table>';
			//}
