...
function listAct(page){
	...
	contents += '<th style="width:200px;">EVAL_TYPE</th>';
	contents += '<th style="width:120px;">SUBMIT_DATE</th>';
	contents += '<th style="width:120px;">SCORE</th>';
	contents += '<th style="width:120px;">ACCESS_IP</th>';
	contents += '<th style="width:80px;">SUBMIT_DUE_DT</th>';
	contents += '<th style="width:80px;">CHANGE_STATE</th>';
	contents += '<th style="width:80px;">IS_COPIED_ANSWER</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+'&joinURL='+joinURL+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
					...
					lists += '<td>'+this.EVAL_TYPE+'</td>';
					lists += '<td>';
					if(this.SUBMIT_DATE == '' || this.SUBMIT_DATE == null){
						lists += '-'
					}else{
						lists += this.SUBMIT_DATE;;
					}
					lists += '</td>';
					lists += '<td>'+this.SCORE+'</td>';
					lists += '<td>'+this.ACCESS_IP+'</td>';
					lists += '<td>';
					if(this.SUBMIT_DUE_DT == '' || this.SUBMIT_DUE_DT == null){
						lists += '-'
					}else{
						lists += this.SUBMIT_DUE_DT;;
					}
					lists += '</td>';
					lists += '<td>'+this.CHANGE_STATE+'</td>';
					lists += '<td>'+this.IS_COPIED_ANSWER+'</td>';
