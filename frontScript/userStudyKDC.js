...
var chapterApi = '../api/apiStudyChapterKDC.php';
    ...
    if ($studyBlock.find('form').length != 0) { //스터디블록 닫기
        ...
        var studyDetails = $.get(chapterApi, 'contentsCode=' + contentsCode + '&lectureOpenSeq=' + lectureOpenSeq, function (data) {
            ...
            */
            ...
            } else if (data.userAuth == 'Y' && data.certPass == 'N' && data.serviceType == 5 && new Date(data.lectureStart) > new Date('2023-05-01') && loginCompanyCode != '3018211347' && loginUserID != 'dhkang99') { // 산업안전 본인인증용
                certPassModal2(data.lectureStart, studySeq, contentsCode, lectureOpenSeq, data.userAuth);
            } else {
                if (data.serviceType == 1 || data.serviceType == 2 || data.serviceType == 3 || data.serviceType == 9 || data.serviceType == 5) { //2017.07.19 강혜림 서비스타입 5 추가
...
//              studyList += '-'
            ...
            studyList += '</tr>';
        } else if (data.serviceType == 1 || data.serviceType == 2 ||data.serviceType == 9 || data.serviceType == 5 || data.serviceType == 6) { //2017.07.19 강혜림 서비스타입 5번 추가
			
            //중간평가 노출
            if (useMidTest != null && (data.serviceType == 1 || data.serviceType == 2 ||data.serviceType == 9 || data.serviceType == 5 || data.serviceType == 6)) {//서비스타입에 따른 노출 지정
                if (midTerm != 0) {
                        ...
                        if (data.midStatus == 'Y' || data.midStatus == 'C') { //채점전
                            ...
                            if(data.serviceType == 5 && /dhkang99|9707262672/.test(loginUserID) && data.passScore > data.testScore ){
                                ...
                                if(data.retakenWay == 0){
                                  studyList += `<br/><button type="button" onclick="retaken('${loginUserID}','${data.seq}','mid',0)" style="font-size:16px; margin-right:0px;">재응시하기</button></td>`;
                                }

                                if(data.retakenWay == 1){

                                  studyList += `<br/><button type="button" onclick="retaken('${loginUserID}','${data.seq}','mid',1)" style="font-size:16px; margin-right:0px;">재응시 요청하기</button></td>`;
                                }
    ...
    //최종평가
    if (useFinTest != null && (data.serviceType == 1 || data.serviceType == 2 ||data.serviceType == 9 || data.serviceType == 5 || data.serviceType == 6)) { //2017.07.19 강혜림 서비스타입 5번 추가
        ...
        if (eval(data.totalProgress) >= eval(data.finalTestProgress)) {
            if ((data.testStatus == 'Y' && data.testOverTime != 'Y') || data.testStatus == 'C') {
              ...
              if( data.serviceType == 5  && parseInt(data.passScore) > parseInt(data.testScore) ){
                ...
                if(data.retakenWay == 0){
                  studyList += `<br/><button type="button" onclick="retaken('${loginUserID}','${data.seq}','test',0)" style="font-size:16px; margin-right:0px;">재응시하기</button></td>`;
                }

                if(data.retakenWay == 1){
                  studyList += `<br/><button type="button" onclick="retaken('${loginUserID}','${data.seq}','test',1)" style="font-size:16px; margin-right:0px;">재응시 요청하기</button></td>`;
                }
    ...
    //
    //레포트
    if (useReport != null && (data.serviceType == 1 || data.serviceType == 2 ||data.serviceType == 9 || data.serviceType == 5)) { //2017.07.19 강혜림 서비스타입 5번 추가
...
function openStudyDiv(className) { // 수강창
    ...
    modalWrite += '<li class="selectedSummary" onclick="changeSummary(this);">학습요점</li><li onclick="changeSummary(this);">학습노트</li>';
	//modalWrite += '<li onclick="changeSummary(this);">문의하기</li>';
	modalWrite += '</ul>';
...
function frameLoad() {
    ...
    if (loginUserID == 'zkfmak3785') {
        window.top.frames[1].postMessage(`12`, `https://${contentsInfo.serverLocation}.modulearning.kr`);
        window.addEventListener('message', getPostMessage);
    }
    clearTimeout(timeCheck);
    timeCheck = setTimeout(step, interval);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            result = {
                ...
                contentsCode: contentsCode,
                lectureOpenSeq: lectureOpenSeq,
                nowPageUrl: data.progress[chapter - 1].lastPage??data.progress[chapter - 1].chapterPath,
                conWidth: conWidth,
                conHeight: conHeight,
