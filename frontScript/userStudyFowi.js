            ...
            $.each(data.study, function () {
                ...
                if (totalCount > 1 && data.studyLimit == 'Y' && (this.serviceType == '3' || this.serviceType == '5')) {
                    ...
                    if (this.serviceType != 8 && this.serviceType != 7) {
                        lists += `<div class="summuryArea" data-seq="${this.seq}" data-contents-code="${this.contentsCode}"
                                   data-lecture-open-seq="${this.lectureOpenSeq}" data-service-type="${this.serviceType}">`;
                ...
                if ((this.serviceType == '1' || this.serviceType == '3' || this.serviceType == '5' || this.serviceType == '10' || this.serviceType == '11') && this.passOK == 'Y') {   //수료시 수료증 출력
                    if (this.serviceType == '5') {
						...
						lists += `<li id="passOK"><h1>수료여부</h1><b style="color:blue">수료</b></li>`;
					}
					// lists += '<li><h1>수료여부</h1><img src="/images/study/img_success.png" onclick="printPop(' + this.seq + ');" style="width:71px; height:60px; margin:6px 15px;" /></li>';
        ...
        document.querySelectorAll('.summuryArea').forEach(ele => {
            if (ele.dataset.seq) {
                ele.addEventListener('click', (e) => {
                    const {seq, contentsCode, lectureOpenSeq, serviceType} = ele.dataset;

                    viewStudy(e, seq, contentsCode, lectureOpenSeq, serviceType);
                })
            }
        })
...
function openStudyTab(studySeq, contentsCode, lectureOpenSeq, serviceType) {
	if (	contentsCode === "H9JMAO" || contentsCode === "FQBCS2" || 
			contentsCode === "0APN00" || contentsCode === "ZK4M1N" || 
			contentsCode === "HAFE81" || contentsCode === "WPDYF0" || 
			contentsCode === "AFFFN0" || contentsCode === "6700KS" || 
			contentsCode === "KZ7VXC" || contentsCode === "2H3B1J" || 
			contentsCode === "8SEVOM" || contentsCode === "U5EBDY" || 
			contentsCode === "DQKS3M" || contentsCode === "IIZMQS" || 
			contentsCode === "OXBF0O") {
		alert('만족도조사, 사후테스트를 완료해야 수료처리가 됩니다. (진도 80%이상 진행 가능)');
	} else if (contentsCode === "203U5O" || contentsCode === "L96BLK" || 
			contentsCode === "IPHQY7" || contentsCode === "AFXJUN" || 
			contentsCode === "STISIX" || contentsCode === "HE725L" || 
			contentsCode === "ZGQ0QK" || contentsCode === "SVSDCN" || 
			contentsCode === "3MQ3TC") {
		alert('만족도조사를 완료해야 수료처리가 됩니다. (진도 80%이상 진행 가능)');
	} else {
		alert('만족도조사, 최종평가 70점 이상 완료해야 수료처리가 됩니다.');
	}
...
function viewStudy(obj, studySeq, contentsCode, lectureOpenSeq, serviceType) {
		
	var surveyCkeck = $.get('../api/apiSurveyCheck.php', {'contentsCode':contentsCode, 'lectureOpenSeq':lectureOpenSeq}, function (data) {
		var surveyCode = data.surveyCode;
		if (serviceType == 3) {
			data.totalCount = 1;
		}
		if (data.totalCount == 0 && (
			contentsCode === "H9JMAO" || contentsCode === "FQBCS2" || 
			contentsCode === "0APN00" || contentsCode === "ZK4M1N" || 
			contentsCode === "HAFE81" || contentsCode === "WPDYF0" || 
			contentsCode === "AFFFN0" || contentsCode === "6700KS" || 
			contentsCode === "KZ7VXC" || contentsCode === "2H3B1J" || 
			...
			contentsCode === "DQKS3M" || contentsCode === "IIZMQS" || 
			contentsCode === "OXBF0O")) {
			window.location.href = `userSurveyPage.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&studySeq=${seq}&serviceType=${serviceType}&surveyCode=${surveyCode}&studySeq=${studySeq}`;
		}
	}).done(function() {
...
function viewStudyDetail(studySeq, contentsCode, lectureOpenSeq) {
    ...
    var studyDetails = $.get(chapterApi, 'contentsCode=' + contentsCode + '&lectureOpenSeq=' + lectureOpenSeq, function (data) {
        ...
        if ((data.serviceType == '1' || data.serviceType == '2') && data.certPass == 'N') {
            ...
            certPassModal2(data.lectureStart, studySeq, contentsCode, lectureOpenSeq, data.userAuth, serviceType);
        } else {
    ...
    fetch(checkTestApi + `?type=${type}&seq=${seq}&userID=${loginUserID}`)
        ...
        .then(data => {
            ...
            if (data.result == 'success') {
                /*if (hasSurvey == 'N' && type == 'final') {
                    //setEmonFormValue(studyObj[contentsCode + ',' + lectureOpenSeq])
                    //getSurveyChk(studyObj[contentsCode + ',' + lectureOpenSeq], `userTestPage.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&like=${type}&studySeq=${seq}&serviceType=${serviceType}`)
                } else {
                    window.location.href = `userTestPage.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&like=${type}&studySeq=${seq}&serviceType=${serviceType}`;
                }*/
				window.location.href = `userTestPage.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&like=${type}&studySeq=${seq}&serviceType=${serviceType}`;
            }
        })
