...
function listAct(page){
	...
	contents += '<th style="width:200px;">TOTAL_SCORE</th>';
	contents += '<th style="width:120px;">PROGRESS_RATE</th>';
	contents += '<th style="width:120px;">CHANGE_STATE</th>';
	contents += '<th style="width:120px;">REG_DATE</th>';
	contents += '<th style="width:80px;">PASS_FLAG</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+'&joinURL='+joinURL+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
					...
					lists += '<td>'+this.TOTAL_SCORE+'</td>';
					lists += '<td>';
					if(this.PROGRESS_RATE == '' || this.PROGRESS_RATE == null){
						lists += '-'
					}else{
						lists += this.PROGRESS_RATE;;
					}
					lists += '</td>';
