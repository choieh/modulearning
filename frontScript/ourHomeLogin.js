...
function loginPage(types, nextAct) {
  ...
  if (types == 'login') {
      ...
      pageWrites += '<img src="../../attach/studyCenter/ourhome_login_logo.png" alt="로그인">';
      pageWrites += '<form id="login" action="javascript:actLogin(\'' + nextAct + '\')">';
      pageWrites += '<h1>교육시스템의 원활한 이용을 위해서는<br>로그인이 필요합니다.</h1>';
      pageWrites += '<input type="text" placeholder="아이디" name="userID">';
      pageWrites += '<input type="password" placeholder="비밀번호" name="pwd">';
      pageWrites += '<button type="submit"><img src="../images/member/btn_login.png" alt="로그인">로그인</button>';
      pageWrites += '</form>';
      pageWrites += '<div id="btn-zone">';
      pageWrites += '<a href="javascript:loginPage(\'findID\')" style="font-weight: bold;">아이디 찾기</a>';
      pageWrites += '<a href="javascript:loginPage(\'findPW\')" style="font-weight: bold;"><b>비밀번호 찾기</b></a>';
      pageWrites += '</div>';

  } else if (types == 'findID') {
      
      pageWrites += '<img src="../../attach/studyCenter/ourhome_login_logo.png" alt="아이디찾기">';
      pageWrites += '<form id="login" action="javascript:findMember(\'ID\')" class="find-idForm">';
      pageWrites += '<h1>회원가입하신 정보와 일치해야 아이디를 찾을 수 있습니다.</h1>';
      pageWrites += '<div><h1 style="line-height:44px;">이름</h1><input type="text" name="userName" style="width:300px;" /></div>';
      pageWrites += '<div><h1 style="line-height:44px;">생년월일</h1><input type="tel" name="birth01" maxlength="2" />&nbsp;년&nbsp;&nbsp;&nbsp;<input type="tel" name="birth02" maxlength="2" />&nbsp;월&nbsp;&nbsp;&nbsp;<input type="tel" name="birth03" maxlength="2" />&nbsp;일&nbsp;</div>';
      pageWrites += '<div><h1 style="line-height:44px;">휴대폰</h1><input type="tel" name="mobile01" maxlength="3" />&nbsp;-&nbsp;<input type="tel" name="mobile02" style="width:60px;" maxlength="4"" />&nbsp;-&nbsp;<input type="tel" name="mobile03" style="width:60px;" maxlength="4"" /></div>';
      pageWrites += '<button type="submit"><img src="../images/member/btn_find.png" alt="찾기" />아이디 찾기</button>';
      pageWrites += '</form>';
      pageWrites += '<div id="btn-zone">';
      pageWrites += '<a href="javascript:loginPage(\'login\',\'main/index.php?\')"><b style="font-weight:bold;">로그인</b></a>';
      pageWrites += '<a href="javascript:loginPage(\'findPW\')" style="font-weight: bold;"><b>비밀번호 찾기</b></a>';
      pageWrites += '</div>';

  } else if (types == 'findPW') {

      pageWrites += '<img src="../../attach/studyCenter/ourhome_login_logo.png" alt="비밀번호찾기">';

      pageWrites += '<form id="loginpw" action="javascript:findMember(\'PW\')" class="find-pwdForm">';
      pageWrites += '<h1>회원가입하신 정보와 일치해야 비밀번호를 찾을 수 있습니다.</h1>';
      pageWrites += '<div><h1>아이디</h1><input type="text" name="userID" /></div>';
      pageWrites += '<div><h1>이름</h1><input type="text" name="userName" /></div>';
