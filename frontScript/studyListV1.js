...
//
//	작성자 : 서영기
var allowUserID = ['eungmin2', 'zkfmak3785', 'dr2201', 'hh19', 'want314', 'sseul0613', 'by1644', 'dnthdgml','sujeongyee', 'wlth5704'];
...
function listAct(page) {
    ...
    if (allowUserID.includes(loginUserID) || loginUserLevel == '7' || loginUserLevel == '8' || loginUserID == '1198638589') {
...
//		actionArea += '<option value="80">수강연동(비환급)</option>';
    ...
    if (loginUserLevel == 8) {
        actionArea += '<div id="contentsNameArea" style="display:inline-block; margin-right:10px;">';
        actionArea += '<select name="contentsName"">';
    ...
    // 240828 재응시요청내역 검색 (박수정 추가)
    actionArea += '<span>재응시요청내역</span>';
    actionArea += '<select name="reTestRequest">';
    actionArea += '<option value="">전체</option>';
    actionArea += '<option value="Y">있음</option>';
    actionArea += '<option value="N">없음</option>';
    actionArea += '</select>';
    ...
    if (loginUserLevel <= 8) {
        if (allowUserID.includes(loginUserID)) {
            ...
            actionArea += ' <button type="button" onClick="excelAct5()" style="margin-left:10px">HRD 업로드</button>'
        }
    ...
    contents += '<th style="width:90px;">재응시요청/<br>재응시처리</th>';
...
function excelAct() {
    ...
    top.location.href = 'progressV1.php?' + searchValue;
}
...
function excelAct2() {
    ...
    window.open('progressBV1.php?' + searchValue);
}
...
function excelAct3() {
    ...
    window.open('progressSpeedV1.php?' + searchValue);
}
...
function excelAct4() {
    ...
    window.open('progress3V1.php?' + searchValue);
}

function excelAct5() {
    searchValue = $('.searchForm').serialize();
    searchValue = '&' + searchValue;
    window.open('hrdUploadExcel.php?' + searchValue);
}
...
function ajaxAct(sortDatas) {
    ...
    if (loginUserLevel == '8') {
        sortData += `&companyCode=${loginCompanyCode}`;
    }
            ...
            $.each(data.study, function () {
                ...
                if (this.user.userRetire == 'Y') {
                    ...
                    if (this.user.ssoName) {
                        ...
                        lists += '<br/><span style="color:blue">ID:' + this.user.ssoKey + '</span>';
                    }
                    lists += '</td>';
                }
                ...
                lists += '<td>';
                // 240827 재응시 요청 날짜, 시간 (박수정 추가)
                if(this.retakenStatus == '' || this.retakenStatus == null) {
                    lists +='x <br>';
                } else {
                    if(this.lastRequest !=='' && this.lastRequest !== null) {
                        lists +='<p style="color:#0780c2;">'+this.lastRequest +' /</p>';
                    } else {
                        lists +='<p style="color:#0780c2;">'+this.inputDate +' /</p>';
                    }     
                }
                ...
                if (loginUserLevel == '8' && this.serviceType == '5') {
                    ...
                    if ((this.testStatus == "Y" || this.testStatus == "C") && this.serviceType != '3' && this.serviceType != '8') {
                        lists += '&nbsp;<button type="button" onClick="retakenEduMng(\'' + this.user.userID + '\',\'' + this.seq + '\',\'test\')">최종</button>';
                    }
                    //응시중일때 상태 "V"
                    if (this.testStatus == "V" && Date.now() > new Date(this.testEndTime).getTime()) {
                        lists += '&nbsp;<button type="button" onClick="retakenEduMng(\'' + this.user.userID + '\',\'' + this.seq + '\',\'test\')">최종</button>';
                    }
                    if ((this.reportStatus == "V" || this.reportStatus == "Y" || this.reportStatus == "C") && this.serviceType != '3' && this.serviceType != '8') {
...
function searchTypeSelect(types, searchDayType) {
    ...
    if (types == 'searchDate') {
        ...
        chageSearch += `<input type="text" name="searchCompany">`;
    ...
    // 2024.06.03 사업주 검색 debounce 적용
    document.querySelector('input[name="searchCompany"]').addEventListener('input',
        debounce((event) => {
            searchStudy('company', event.target)
        }, 300)
    )
        ...
        if (searchName != '' && searchName != ' ') {
            $.get(chainsearchApi, {
                ...
                if (data.totalCount != 0) {
                    ...
                    selectWrite += `<strong class="noticeSearch price">&nbsp;&nbsp;&nbsp;검색결과가 없습니다.</strong>`
                }
                $('select[name="companyCode"], strong.noticeSearch').remove();
...
async function retaken(userID, seq, lectureOpenSeq, cotentsCode, testType, testStatus, serviceType) {
    if (testType == 'mid') {
        var testTxt = '중간평가';
...
function searchContents (searchDate) {
    var year = $('select[name="searchYear"]').val().replace('년', '');
    var month = $('select[name="searchMonth"]').val().replace('월', '');
    month = month.padStart(2, '0'); 

    var lectureDay = loginCompanyCode + '/' + year + '-' + month + '-01~'+year + '-' + month +'-31';
    var searchType = 'monthContents';

    if(month === '00') {
        lectureDay = loginCompanyCode +'/' + year + '-01-01~'+year + '-12-31';
    }
    
    if(searchDate !== '' && searchDate != null) {
        lectureDay = loginCompanyCode + '/' +searchDate;
        searchType = 'dateContents';
    }
    
    $.get(chainsearchApi, {
        'searchMode': 'study',
        [searchType] : lectureDay,
        'type': 'contents'
    }, function (data) {
        var selectWrite = '';
        if (data.totalCount != 0) {
            selectWrite += '<select name="contentsName">';
            selectWrite += '<option value="">과정을 선택해주세요</option>'
            $.each(data.searchResult, function () {
                selectWrite += '<option value="' + this.contentsName + '">' + this.contentsName + '</option>';
            })
            selectWrite += '</select>';
        } else {
            selectWrite += '<strong class="noticeSearch price">&nbsp;&nbsp;&nbsp;검색결과가 없습니다.</option>'
        }
        $('#contentsNameArea').html(selectWrite);
    })
}
