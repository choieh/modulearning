...
function ajaxAct(listPage,sortData){
	...
	var listAjax = $.get(useApi,'enabled=Y&page='+page+'&list='+listCount+sortData,function(data){
		...
		if(totalCount == 0){
			$('#titleArea > h3 > span').html('신청한 과정이 없습니다.');
		}else{
			$('#titleArea > h3 > span').html('총 '+totalCount+'개의 과정을 신청하셨습니다.');
		}		
		var lists = '';
		var i = 1;
		if(page != 1){
		...
		if (totalCount != 0){
			...
			lists += '<li class="noList">신청한 과정이 없습니다.</li>';
		}
...
function studyPop(contentsCode){
	popupAddress = 'https://cont1.modulearning.kr/player/popupConfirm.php?contentsCode='+contentsCode+'&chapter=1';
	window.open(popupAddress,"모두의러닝학습창","menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","esangStudy")
}
