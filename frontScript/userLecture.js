...
var useApi             = '../api/apiLectureContentsMapping.php';
...
var lectureCategoryApi = '../api/apiLectureCategory.php';
...
function listAct(page){
  ...
  actionArea += '<input type="text" name="searchValue" />&nbsp;';
	...
	.done(function(){
		$.get(lectureCategoryApi,{'companyID':subDomain},function(data){
			var leftMenu = ''
			$.each(data.category, function(){
				var conNum = Number(this.value01.slice(7))
				leftMenu += '<li onclick="top.location.href=\'/lecture/?sort01='+this.value01+'\'">';
				leftMenu += this.value01+'과정';
				leftMenu += '</li>';
				if(this.value01 == sort01){
					//$('#titleArea > h2 strong, #titleArea > h1').html(this.value01+'과정');
				}
				categoryName01[conNum] = this.value01;
				/*
...
function lectureSubmit(types,contentsCode,contentsName,price,lectureStart,lectureEnd){
	...
	if(loginUserID == ''){
		...
		//if(subDomain != ''){
			...
			modalWrite +='자세한 사업주지원훈련 정보는 <a href="http://yechanacademy.com/eduinfo" target="_blank">이곳을 클릭</a>하여 확인하실 수 있습니다. 반드시 신청 정보를 확인 후 신청하시기 바랍니다.';
			modalWrite +='</div>';
			modalWrite +='</li>';
		//}
		*/
	...
	$.get(useApi,{'seq':seq,'contentsCode':contentsCode},function(data){
		...
		$.each(data.contents, function(){
			if(sort01 == ''){
				...
				//$('#titleArea > h2 strong, #titleArea > h1').html('과정상세보기');
				sort01 = this.sort01;
				sort02 = this.sort02;
