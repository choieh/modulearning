...
var chapterApi = '../api/apiStudyChapterSSO.php';
    ...
    var listAjax = $.get(useApi, function (data) {
        ...
        if (totalCount != 0) {
            $.each(data.study, function () {
                ...
                }else if((this.serviceType == '3' || this.serviceType == '5') && this.totalScore != null && this.totalScore < this.passScore){
                    lists += '<li><h1>점수</h1><p onclick="reTestButton();"style="font-size:13px;"><strong style="color:red;font-size:14px;">미수료 ['+this.totalScore+'점]</strong></p></li>';
                }               
    ...
    if ($studyBlock.find('form').length != 0) { //스터디블록 닫기
        ...
        var studyDetails = $.get(chapterApi, 'contentsCode=' + contentsCode + '&lectureOpenSeq=' + lectureOpenSeq, function (data) {
            ...
            */
			...
			uSeq = data.seq;
            uLectureOpenSeq = data.lectureOpenSeq;
            uContentsCode = data.contentsCode;
            uTestStatus = data.testStatus;
			uPassOK = data.passOK;
            ...
            } else if (data.userAuth == 'Y' && data.certPass == 'N' && data.serviceType == 5 && new Date(data.lectureStart) > new Date('2023-05-01')) { // 산업안전 본인인증용
...
//버튼 변환스크립트
function writeBtn(btnType, chapter, progress) {

    chapter = chapter ? chapter : '';
    var writes = '';

	if (progress == false) {
		var btnCss = 'style="background-color: #a5a9aa; border: 1px solid #a5a9aa"';
		var btnTypeStudy = '수강완료';
	} else {
		var btnCss = '';
		var btnTypeStudy = '수강하기';
	}

    if (btnType == 'study') { //학습하기 관련
		
        if (typeof (chapter) == 'number') {			
            writes += '<button class="btn_study" '+btnCss+' type="button" title="수강하기" onclick="studyModal(this,' + chapter + ',\'new\')">';
        } else {
            writes += '<button class="btn_study" '+btnCss+' type="button" title="수강하기" onclick="alert(\'' + chapter + '\')">';
        }
        // writes += '<img src="../images/study/btn_study.png" alt="학습하기" /></button>';
		
        writes += btnTypeStudy+'</button>';

    } else if (btnType == 'continue') {//이어보기 관련

        writes += '<button type="button" '+btnCss+' title="이어보기" onclick="studyModal(this,' + chapter + ')">';
        // writes += '<img src="../images/study/btn_continuestudy.png" alt="이어보기" /></button>';
        writes += '이어보기</button>';

    } else if (btnType == 'midTest' || btnType == 'finTest') {//중간,최종평가관련
        if (btnType == 'midTest') {
            writes += '<button class="btn_dotest" type="button" title="평가응시" onclick="chkTest(\'mid\',this);">';
        } else {
            writes += '<button class="btn_dotest" type="button" title="평가응시" onclick="chkTest(\'final\',this);">';
        }
        // writes += '<img src="../images/study/btn_dotest.png" alt="평가응시" /></button>';
        writes += '평가응시</button>';

    } else if (btnType == 'reportWrite') {//과제응시관련
        writes += '<button type="button" class="btn_dotest" title="과제 제출" onclick="chkTest(\'report\',this);">';
        ...
        if (chapter == 'Y') {
			...
			if (uPassOK == 'Y') {
				writes += `<button type="button" class="btn_dotest" title="과제 제출" >응시완료</button>`;
			} else {
				writes += `<button type="button" onclick="retaken('${loginUserID}','${uSeq}','${uLectureOpenSeq}','${uContentsCode}','test','${uTestStatus}','${serviceType}')" style="text-align:center; color:#ffffff; text-decoration: none;  padding: 4px 12px; font-size: 12px; background: #fa5a5a; margin:5px;">재응시</button>`;
			}
		}
...
//  chkTable +='</div>';
    ...
    chkTable += '<td colspan="6" class="title" style="background-color:#687EFF;">수료기준</td>'
	...
	if(data.totalPassMid == 0 || data.serviceType == 3) {
    ...
    chkTable += '</td>';
    if(data.totalPassTest == 0 || data.serviceType == 3) {
        chkTable += '<td>평가없음&nbsp;<strong>';
    } else {
        chkTable += '<td>총&nbsp;<strong>';
    }
...
function chkTestArea(data, writeTarget) {//평가진행 관련 버튼ul
    ...
    var finMsg = '';
    var finLink = '';
    if (data.serviceType == 3 && data.contentsCode != "3FO31N" && data.companyCode != "5058203997" && data.companyCode != "2198201220" && data.companyCode != "2198201221" && data.companyCode == "2728101049") {		
        fintext = '평가 없는 과정'
...
function runStudyList(data, writeTarget) {
	...
	studyList += '<tr style="font-weight: normal;"><td>차시</td><td>차시명<br/>교육이수시간</td><td>진도율</td><td></td><td>이어보기</td><td>수강하기</td></tr>';

    //수강 노출시작
    var i = 0;
    for (i = 0; i < data.totalCount; i++) {

        //$('.lectureForm'+writeTarget).attr('action',data.progress[i].player+'/player/popupStudy.php');
        if (i != midTerm || data.totalPassMid == 0) { //2018-03-23 이응민 data.totalPassMid == 0 조건 추가 (중간평가가 없고 최종평가만 있을시)
			...
			//if (loginUserID == 'eungmin2') {
				...
				if (data.progress[i].progress != 0) {
				  studyList += writeBtn('continue',(i+1),data.progress[i].progress < 100);
				} else {
				  studyList += '-'
				}
				studyList += '</td>'
            ...
            if (data.serviceType == 1) { // || data.serviceType == 3 ||data.serviceType == 5
                if (studyCnt >= maxStudyCnt) {//최대차시
                    studyList += writeBtn('study', '사업주 직업능력개발훈련 지원규정 1일 학습 가능한 차시(' + maxStudyCnt + '차시)를 초과하였습니다.',data.progress[i].progress < 100);
                } else if (eval(midTerm) != 0 && (eval(data.progress[i].chapter) > eval(midTerm)) && (useMidTest != 'Y' && useMidTest != 'C') && eval(data.serviceType) == 1) {//학습하기
                    studyList += writeBtn('study', '중간평가 응시 완료 후에 학습할 수 있습니다.',data.progress[i].progress < 100);
                } else {
                    studyList += writeBtn('study', (i + 1),data.progress[i].progress < 100);
                }
            } else {
                studyList += writeBtn('study', (i + 1),data.progress[i].progress < 100);
            }
    ...
    if (useFinTest != null && (data.serviceType == 1 || data.serviceType == 9 || data.serviceType == 5)) { //2017.07.19 강혜림 서비스타입 5번 추가
		...
		if (data.testStatus == 'C') {
			studyList += '<td></td><td onClick="testViewPop(\'testResultView\',' + data.lectureOpenSeq + ',\'' + loginUserID + '\',\'final\')" style="cursor:pointer;">'+data.totalScore+'점<br>'+data.testSaveTime+'<br><b style="color:blue">(답안확인)</b></td></td>'
		} else {
			studyList += '<td></td><td></td>';
		}
    ...
    if (useFinTest != null && data.serviceType == 3 && data.contentsCode == "3FO31N" && (data.companyCode == "5058203997" || data.companyCode == "2198201220" || data.companyCode == "2198201221" || data.companyCode == "2728101049" )) { 
        ...
        //기말평가 버튼
        if (data.testStatus == 'Y' || data.testStatus == 'C' || (data.testStatus = 'V' && data.nowTime >= data.testEndTime)) {
            studyList += writeBtn('finTestComplete',data.passOK);
        } else {
            studyList += writeBtn('finTest',data.passOK);
        }
        studyList += '</td>'
        studyList += '</tr>';
    }
...
function sendProgress(open) {
    const nowPageUrl = document.getElementById('frame').src;
    fetch(progressApi + `?userID=${loginUserID}&contentsCode=${contentsInfo.contentsCode}&chapter=${contentsInfo.chapter}`, {
        method: "POST",
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: `contentsCode=${contentsInfo.contentsCode}&lectureOpenSeq=${contentsInfo.lectureOpenSeq}&chapter=${contentsInfo.chapter}&totalTime=${totalTime}&open=${open}&cookie=${document.cookie}&nowPageURL=${nowPageUrl}`,
        keepalive: true
    })
...
function changeSummary(li) {
    ...
    if ($(li).index() == '0') {
        if (contentsInfo.attachFile != '') {
            summaryContent += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습자료받기</span><br>
            ...
            summaryContent += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습자료받기</span><br>등록된 학습자료가 없습니다.</p><br>`;
        }
        summaryContent += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">내용전문가</span><br>${contentsInfo.professor.replace(/\n/g, '<br />')}</p><br>`;
        summaryContent += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습목표</span><br>${contentsInfo.goal.replace(/\n/g, '<br />')}</p><br>`;
        summaryContent += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습내용</span><br>${contentsInfo.content.replace(/\n/g, '<br />')}</p><br>`;
    } else if ($(li).index() == '1') {
        ...
        summaryContent += '<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">문의하기</span></p><br>';
        summaryContent += '<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; ">질문내용 및 답변은 <font style="color:#0780c2;">내 강의실 > 상담신청내역</font>에서 확인 가능합니다.</span></p><br>';
...
function openStudyDiv(className,type) { // 수강창
    ...
    modalWrite += '<div >';
    modalWrite += `<span style="display:block; background:#f2f6f7; text-align:right; padding: 8px 10px 8px 3px; cursor:pointer;" onClick="studyEnd('${className}')">
    <img src="../images/admin/btn_close.png"></span>`;
	...
	if (type != 'new') {
		modalWrite += `<iframe id="frame" name="frame" src="${contentsInfo.lastPageUrl}" style="width:inherit; height:inherit;" onload="frameLoad()" allowfullscreen></iframe>`;
	} else {
    ...
    modalWrite += '<div class="modalSide" style="width:350px; border-left: 1px solid; background:#f2f6f7; box-sizing: border-box;">';
    modalWrite += '<div id="time" style="text-align:center; padding:10px 0; font-size:15px; background:#0780c2; color:white; line-height: 20px;">수강시간&nbsp;<span style="font-size:18px; margin-left: 6px;">00:00:00</span></div>';
    modalWrite += '<div style="padding: 10px; font-size:16px; line-height: 20px;">';

    modalWrite += `${contentsInfo.contentsName}<br>`;
    modalWrite += '<div style="font-size:14px; padding-top:10px;">';
    modalWrite += `${contentsInfo.chapter}차시 | ${contentsInfo.chapterName}`;
    modalWrite += '</div>';
    modalWrite += '</div>';
    modalWrite += `<div style="text-align:center; padding-bottom:20px;">
    <button onclick="studyEnd('${className}')" style="width:90%; height:40px; margin-top:20px; font-size:16px; background:#4C6EFC; font-weight: 600; border-radius:3px; color:white; letter-spacing:2px;">학습종료</button></div>`;
    ...
    //modalWrite += '<li class="selectedSummary" onclick="changeSummary(this);">학습요점</li><li onclick="changeSummary(this);">학습노트</li><li onclick="changeSummary(this);">문의하기</li>';
	modalWrite += '</ul>';
    modalWrite += '<div id="summaryContent" style="font-size:14px; background:white; max-height:230px; padding: 15px 5px; overflow:auto; ">';
    if (contentsInfo.attachFile != '') {
        modalWrite += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습자료받기</span><br>
        <a href="https://modulearning.kr/lib/fileDownLoad.php?fileName=${encodeURI(contentsInfo.attachFile)}&link=${encodeURIComponent('/attach/contents/' + contentsInfo.attachFile)}" target="_blank">
        ${contentsInfo.attachFile}</a>
        </p><br>`;
    } else {
        modalWrite += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습자료받기</span><br>등록된 학습자료가 없습니다.</p><br>`;
    }
    modalWrite += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">내용전문가</span><br>${contentsInfo.professor.replace(/\n/g, '<br />')}</p><br>`;
    modalWrite += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습목표</span><br>${contentsInfo.goal.replace(/\n/g, '<br />')}</p><br>`;
    modalWrite += `<p><span style="display:inline-block; font-size:15px; padding-bottom:6px; color:#0780c2;">학습내용</span><br>${contentsInfo.content.replace(/\n/g, '<br />')}</p><br>`;
    modalWrite += '</div>';
    modalWrite += '</div>';
    if (contentsInfo.serviceType != '5') {
        modalWrite += `<div style="text-align:center; padding:10px 0px; color:#0780c2; line-height: 20px;">진도율은 최소 ${contentsInfo.progressCheckTime} 이상<br> 학습하셔야 정상 반영됩니다.</div>`;
    }
    modalWrite += '</div>';
    modalWrite += '</div>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            result = {
                ...
                contentsUrl: data.progress[chapter - 1].player + data.progress[chapter - 1].chapterPath,
                ...
                contentsCode: contentsCode,
                lectureOpenSeq: lectureOpenSeq,
                nowPageUrl: data.progress[chapter - 1].chapterPath,
                conWidth: conWidth,
                conHeight: conHeight,
				...
				lastPageUrl: data.progress[chapter - 1].lastPage
            }
        }
    })
...
async function studyModal(obj, chapter, type) { // 모달 창 오픈
    var runningForm = '';
    if (typeof (obj) == 'object') {
        runningForm = $(obj).parents('form');
    } else {
    ...
    openStudyDiv(runningForm[0].className,type);
}
...
function testViewPop (types, modalSeq, eachID, option) {
    popupAddress = 'testView.php?types=' + types + '&modalSeq=' + modalSeq + '&eachID=' + eachID + '&option=' + option;
    window.open(popupAddress, "결과보기", "top=0, left=0, width=1500, height=1000, menubar=no, status=no, titlebar=no, toolbar=no, scrollbars=yes, resizeable=no", "reviewPop")
}
