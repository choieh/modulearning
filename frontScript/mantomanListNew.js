...
function listAct(page){
		...
		countList = $.get(useApi,{'type':type},function(data){
			var actionArea = '';
			var consult = '';
			var status = '';
			var learnCount = '';
			...
			if(pageMode != 'userPage'){
				...
				if (type == 'mate') {
					actionArea += '<option value="파트너/러닝메이트">파트너/러닝메이트</option>';
				} else {
					actionArea += '<option value="디지털융합">디지털융합</option>';
					actionArea += '<option value="디지털융합 (테스트)">디지털융합 (테스트)</option>';
					actionArea += '<option value="디지털융합 (닥터한)">디지털융합 (닥터한)</option>';
					actionArea += '<option value="디지털융합 (노정인 팀장)">디지털융합 (노정인 팀장)</option>';
					actionArea += '<option value="디지털융합 (김재원 세무사)">디지털융합 (김재원 세무사)</option>';
					actionArea += '<option value="디지털융합 (이준호 이사)">디지털융합 (이준호 이사)</option>';
					actionArea += '<option value="법정의무교육">법정의무교육</option>';
                    actionArea += '<option value="법정의무교육(첨단)">법정의무교육(첨단)</option>';
                    actionArea += '<option value="법정의무교육(KIBA)">법정의무교육(KIBA)</option>';
                    actionArea += '<option value="법정의무교육(디지털광고협회)">법정의무교육(디지털광고협회)</option>';
					actionArea += '<option value="휴램제휴">휴램제휴</option>';
					actionArea += '<option value="전경련">전경련</option>';
					actionArea += '<option value="중소기업지사">중소기업지사</option>';
					...
					actionArea += '<option value="AI휴먼콘텐츠제작">AI휴먼콘텐츠제작</option>';
					actionArea += '<option value="장애인관리">장애인관리</option>';
					actionArea += '<option value="AI활용세미나">AI활용세미나</option>';
					actionArea += '<option value="법정의무교육(한사진)">법정의무교육(한사진)</option>';
					actionArea += '<option value="법정의무교육(모두)">법정의무교육(모두)</option>';
					actionArea += '<option value="모두비edm">모두비edm</option>';
...
function ajaxAct(sortDatas){
	...
	var listAjax = $.get(useApi,'type='+type+'&viewType='+viewType+'&page='+page+'&list='+listCount+'&boardType='+boardType+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.consult, function(){
				...
				lists += this.addItem02 != null ? '<td>'+this.addItem02+'</td>' : '<td></td>';
