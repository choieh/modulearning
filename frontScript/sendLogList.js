...
function listAct(page){
	...
	actionArea += '<select id="sendMethod" name="sendMethod">';
	...
	actionArea += '&nbsp;&nbsp;&nbsp;<button type="button" onclick="excelAct()">엑셀다운로드</button>';
	actionArea += '</div>';
...
function excelAct() {
	var excelData = $('.searchForm').serialize();
	var sendMethod = $('#sendMethod').val();
	if(sendMethod == 'email'){
		alert('이메일은 지원하지 않습니다.');
		return false;
	}
	window.open('excelSendLog.php?'+'&'+excelData);
}