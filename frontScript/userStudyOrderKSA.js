	...
	var listAjax = $.get(useApi,'enabled=Y&page='+page+'&list='+listCount+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.order, function(){
				...
				lists += '<h4>수강 신청일 : '+this.orderDate+'</h4>'
				lists += '<h4>수강기간 : <strong class="blue">승인일로부터 1개월</strong></h4>';
			})
		} else {
...
function studyPop(contentsCode){
	popupAddress = 'http://cont1.modulearning.kr/player/popupConfirm.php?contentsCode='+contentsCode+'&chapter=1';
