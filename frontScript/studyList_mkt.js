...
function listAct(page){
  ...
  actionArea += '</select>';
	...
	if(loginUserLevel < '5' && loginUserLevel != '4') {
		contents += '<th style="width:80px;">교ㆍ강사<!--<br />영업담당자--></th>';
	}
...
function ajaxAct(sortDatas){
	...
	var listAjax = $.get(useApi2,'page='+page+'&list='+listCount+'&serviceType='+serviceType+sortData,function(data){
		...
		if (totalCount != 0 && loginUserLevel <= userLevel){
			$.each(data.study,  function(){
				...
				lists += '</td>';
				if(loginUserLevel < '5') {
					lists += '<td><button type="button" onClick="deleteData(useApi2,'+this.seq+')">삭제</button></td>';
				}
				lists += '</tr>';
				j--;
			})
...
function searchStudy(types,vals,searchDayType){
	...
	if(types=='lectureDay'){
		...
		$.get(chainsearchApi2,{'searchMode':types, 'searchDay':dateChain, 'searchDayType':searchDayType},function(data){
			var selectWrite = ''
		...
		if( searchName != ''&& searchName != ' ' ){
			$.get(chainsearchApi2,{'searchMode':types, 'searchName':searchName},function(data){
				var selectWrite = ''
				if(data.totalCount !=0){
		...
		$.get(chainsearchApi2,{'searchMode':'study', 'lectureDay':searchDate},function(data){
			var selectWrite = ''
			if(data.totalCount !=0){
		...
		$.get(chainsearchApi2,{'searchMode':'study', 'companyCode':companyCode, 'type':'contents'},function(data){
			var selectWrite = ''
			if(data.totalCount !=0){
		...
		var companyCode = vals.value;
		$.get(chainsearchApi2,{'searchMode':'study', 'companyCode':companyCode},function(data){
			var selectWrite = ''
			if(data.totalCount !=0){				
		...
		$('strong.noticeSearch, select[name="contentsCode"]').remove();
		var searchDate = vals.value;
		$.get(chainsearchApi2,{'searchMode':'study', 'lectureDate':searchDate, 'type':'contents'},function(data){
			var selectWrite = ''
			if(data.totalCount !=0){
				selectWrite += '<select name="contentsCode" onChange="searchAct()">';
				selectWrite += '<option value="">과정을 선택해주세요</option>'
