				...
				if(data.certPass == 'Y' || data.serviceType == '3' || data.serviceType == '9' ){
					...
					for (i = 0 ; i<totalCount; i++){
						...
						if (data.serviceType == 1 || data.serviceType == 9){  // 환급 과정일때만 평가 항목 출력
							...
							if( btnUse == 'Y' && todayCount <= 8 ){
								if( i == midTerm && midTestView == 'N'){
									studyDetails += '<li onclick="alert(\'평가는 PC에서만 가능합니다.\')">';
									//studyDetails += '<button type="button"><img src="../images/mobile/btn_play.png"></button>';
								}else if(i == midTerm && midTestView == 'Y' && data.midStatus == 'N'){
									studyDetails += '<li onclick="alert(\'중간평가 응시완료 후 다음 차시 학습이 가능합니다. PC에서만 응시 가능합니다.\')">';
									studyDetails += '<button type="button"><img src="../images/mobile/btn_play.png"></button>';
								}else {
									if(i%8 == 0 || i == 0){
										if(today2 >= '2017-08-16 18:00:00' && today2 <= '2017-08-18 09:00:00') {
											...
											studyDetails += '<li onclick="top.location.href=\'captcha.php?type=study&chapter='+data.progress[i].chapter+'&contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq+'&studySeq='+studySeq+'&sourceType='+sourceType+'\'">';
										}
									}else {
							...
							if( i == midTerm && midTestView == 'N' && data.midStatus != null ){
								studyDetails += '<h3><strong>평가</strong><br />중간평가</h3>';
								studyDetails += '<h1>중간평가</h1>';
								studyDetails += '<h2>';
								switch(data.midStatus){
									case 'N': studyDetails += '미응시';
									break;

									case 'V': studyDetails += '응시중';
									break;

									case 'Y': studyDetails += '응시완료';
									break;

									case 'C': studyDetails += '채점완료';
									break;
								}
								studyDetails += '</h2>';
							}else{
					...
					if (data.serviceType == 1 || data.serviceType == 9){  // 환급 과정일때만 평가 항목 출력
						if(data.testStatus != null){
							studyDetails += '<li onclick="alert(\'평가는 PC에서만 가능합니다.\')">';
							studyDetails += '<button type="button"><img src="../images/mobile/btn_play.png"></button>';
							studyDetails += '<h3><strong>평가</strong><br />최종평가</h3>';
							studyDetails += '<h1>최종평가</h1>';
							studyDetails += '<h2>';
						...
						if(data.reportStatus != null){
							studyDetails += '<li onclick="alert(\'평가는 PC에서만 가능합니다.\')">';
							studyDetails += '<button type="button"><img src="../images/mobile/btn_play.png"></button>';
							studyDetails += '<h3><strong>과제</strong><br />과제제출</h3>';
							studyDetails += '<h1>과제제출</h1>';
							studyDetails += '<h2>';
                    ...
                    var studyDetails = $.get(chapterApi,'contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq,function(data){
                        location.href = 'studyModal.html?startDate='+data.lectureStart+'&studySeq='+studySeq+'&contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq+''
                    })
				}
			}else{
	...
	$.get('../api/apiLoginUser.php',function(data){
		if(data.userID == '') {
			...
			if(sourceType=='book') {
				...
				var studyDetails = $.get(chapterApi,'contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq+'&progress='+chapter,function(data){
					if(check != ''){
					...
					var linkAddress = data.progress[0].player+'/player/popup_html.php?studySeq='+studySeq+'&contentsCode='+contentsCode+'&chapter='+chapter+'&lectureOpenSeq='+lectureOpenSeq+'&check='+check;
					top.location.href = linkAddress;
				})
			} else {
				if(confirm('3G/4G 환경에서는 데이터 요금이 발생할 수 있습니다.')){
					if(confirm('이어보기를 하시겠습니까?')){
						check = check
					}else{
						check = ''
					}
