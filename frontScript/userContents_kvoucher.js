	...
	/*.done(function(){
		...
		$.get(categoryApi,{'value01':'lectureCode','companyID':subDomain},function(data){
			...
			$.each(data.category, function(){
				...
				leftMenu += '<li onclick="top.location.href=\'/flipped_learning/?sort01='+this.value01+'\'">';
		...
		.done(function(){
			if(sortData == ''){
				$.get(categoryApi,{'value01':'lecture14','enabled':'Y'},function(data){
					var tabMenu = ''
					if(data.division != 0) {
			...
			$.each(data.contents, function(){
				...
				if(startTime <= nowTime && endTime >= nowTime){
					if(subDomain == 'spinelove'){							
						...
						// if(loginUserID == ''){
							// lists += '<button type="button" onClick="alert(\'로그인 후 이용해주세요. 로그인 페이지로 이동합니다.\'); location.href=\'http://symc.modulearning.kr/studyCenter/login.php\'"><img src="../images/lecture/img_button_submit.png" alt="신청하기이미지"><br />신청하기</button>';
                        ...
                        // }else {
                            if(loginUserID == 'ksk8948'){
                                console.log(this.contentsName + ' : ' +this.serviceType);
                                //사업주환급의 경우 신청 후 관리자승인 이후 결제
