		...
		var studyDetails = $.get(chapterApi,'contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq,function(data){
			...
			if (totalCount != 0){
				...
				if(certPassQuery || data.serviceType == '3' || data.serviceType == '9' || data.serviceType == '5'){
					...
					studyDetails +='<li style="padding:0;"><ul class="progress progress_wrap1" onclick="alert(\'PC에서만 응시 가능 합니다.\')">';
					studyDetails +='<li class="type01"><h1>중간평가<br /><strong>'+midStatus+'</strong><br /><strong>'+midComment+'</strong></h1></li>';
					studyDetails +='<li class="type02"><h1>최종평가<br /><strong>'+testStatus+'</strong><br /><strong>'+testComment+'</strong></h1></li>';
					studyDetails +='<li class="type03"><h1>과제제출<br /><strong>'+reportStatus+'</strong><br /><strong>'+reportComment+'</strong></h1></li>';
					studyDetails +='</ul></li>';


					/*진도율, 랭킹 출력*/
					studyDetails +='<ul class="progress progress_wrap2">';
					studyDetails +='<li class="type01 per01"><h1>권장진도율<br /><strong>'+data.suggestProgress+'<span>%</span></strong></h1><div><p style="width:'+data.suggestProgress+'%"></p></div></li>';
					studyDetails +='<li class="type02 per02"><h1>내 진도율<br /><strong>'+data.totalProgress+'<span>%</span></strong></h1><div><p style="width:'+data.totalProgress+'%"></p></div></li>';
					studyDetails +='<li class="type03 per03"><h1>평균진도율<br /><strong>'+data.aveProgress+'<span>%</span></strong></h1><div><p style="width:'+data.aveProgress+'%"></p></div></li>';
					studyDetails +='</ul>';
									...
									if(today2 >= '2018-12-14 13:00:00' && today2 <= '2018-12-14 18:00:00') {	//캡차 제외 처리
										...
										if(data.serviceType == 1 || data.serviceType == 2) {
											//if(loginUserID == 'zeroha3416' || loginUserID == 'eungmin2') {
												studyDetails += '<li onclick="top.location.href=\'hrdOTP_mobile.php?mobileYN=Y&type=study&chapter='+data.progress[i].chapter+'&contentsCode='+contentsCode+'&lectureOpenSeq='+lectureOpenSeq+'&studySeq='+studySeq+'&mobileSourceType='+mobileSourceType+'&progressCheck='+progressCheck+'&lectureStart='+data.lectureStart+'\'">';
											//} else {
