...
function listAct(page){
	...
	.done(function(){
		...
		$.get(categoryApi,{'value01':'lectureCode', 'value03':'사업주', 'companyID':subDomain},function(data){
		...
		if (totalCount != 0){	
			...
			$.each(data.contents, function(){
				...
				if(startTime <= nowTime && endTime >= nowTime){
					...
					//if(this.inputStudyType == 'appInput'){						
				...
				if(subDomain == 'boram') {
					...
					lists += '<h3>'+this.sort01Name+' > '+this.sort02Name;
...
//			}
			...
			if(subDomain == 'boram') {
				...
				views += '<h5>'+this.sort01Name+' <img src="../images/global/icon_triangle.png" alt="화살표" /> '+this.sort02Name;
				if(this.serviceType != '1'){
					views += '(비환급 과정)';
				}
				views += '</h5>';
