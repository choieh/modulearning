        ...
        if (totalCount != 0) {
            $.each(data.study, function () {
				...
				if (this.totalScore > 0) {
					var totalS = this.totalScore;
				} else {
					var totalS = '-';
				}
				lists += '<li class="smallText"><h1>점수/진도율</h1><strong>' + totalS + '</strong>/ <br /><strong>' + this.progress + '</strong>%</li>';
...
function writeBtn(btnType, chapter, progress) {
    ...
    if (btnType == 'study') { //학습하기 관련
        ...
        if (typeof (chapter) == 'number') {			
            writes += '<button class="btn_study" '+btnCss+' type="button" title="복습하기" onclick="studyModal(this,' + chapter + ',\'new\')">';
        } else {
            writes += '<button class="btn_study" '+btnCss+' type="button" title="복습하기" onclick="alert(\'' + chapter + '\')">';
        }
		...
		writes += `<button type="button" class="btn_dotest" style="background-color: #a5a9aa; border: 1px solid #a5a9aa" title="응시완료" >응시완료</button>`;		

    } else if (btnType == 'reportComplete') {//과제제출완료관련
        writes += `<button type="button" class="btn_dotest" style="background-color: #a5a9aa; border: 1px solid #a5a9aa" title="제출완료" >제출완료</button>`;
    }
    return writes;
}
...
function runStudyList(data, writeTarget) {
	...
	studyList += '<tr style="font-weight: normal;"><td>차시</td><td>차시명<br/>교육이수시간</td><td>진도율</td><td></td><td>이어보기</td><td>복습하기</td></tr>';
        ...
        if (i != midTerm || data.totalPassMid == 0) { //2018-03-23 이응민 data.totalPassMid == 0 조건 추가 (중간평가가 없고 최종평가만 있을시)
			...
			//if (loginUserID == 'eungmin2') {
				...
				if (data.progress[i].progress != 0) {
				  studyList += writeBtn('continue',(i+1),data.progress[i].progress != 0);
				} else {
				  studyList += '-'
				}
				studyList += '</td>'
            ...
            if (data.serviceType == 1) { // || data.serviceType == 3 ||data.serviceType == 5
                if (studyCnt >= maxStudyCnt) {//최대차시
                    studyList += writeBtn('study', '사업주 직업능력개발훈련 지원규정 1일 학습 가능한 차시(' + maxStudyCnt + '차시)를 초과하였습니다.',data.progress[i].progress != 0);
                } else if (eval(midTerm) != 0 && (eval(data.progress[i].chapter) > eval(midTerm)) && (useMidTest != 'Y' && useMidTest != 'C') && eval(data.serviceType) == 1) {//학습하기
                    studyList += writeBtn('study', '중간평가 응시 완료 후에 학습할 수 있습니다.',data.progress[i].progress != 0);
                } else {
                    studyList += writeBtn('study', (i + 1),data.progress[i].progress != 0);
                }
            } else {
                studyList += writeBtn('study', (i + 1),data.progress[i].progress != 0);
            }
