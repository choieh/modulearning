...
function listAct(page){
    ...
    actionArea += '<option value="SIMSA_CODE">심사코드</option>';
    actionArea += '</select>&nbsp;';
	...
	contents += '<th style="width:200px;">NAME</th>';
	contents += '<th style="width:120px;">TOTAL_EDU_TIME</th>';
	contents += '<th style="width:200px;">SIMSA_CODE</th>';
	contents += '<th style="width:120px;">VALID_FLAG</th>';
	contents += '<th style="width:120px;">POST_COURSE_FLAG</th>';
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
				console.log(data.data);
					...
					lists += '<td>'+this.TOTAL_EDU_TIME+'</td>';
					lists += '<td>';
					if(this.SIMSA_CODE == '' || this.SIMSA_CODE == null){
						lists += '-'
					}else{
						lists += this.SIMSA_CODE;;
					}
					lists += '</td>';
					lists += '<td>';
					if(this.VALID_FLAG == ''){
