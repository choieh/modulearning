...
var useApi = `../api/apiOurHomeContents.php?type=${type}`;
      ...
      $.get(categoryApi, {'value01': 'lectureCode', 'companyID': subDomain}, function (data) {
        ...
        $.each(data.category, function () {
          ...
          if (this.value01 == sort01) {
            if (subDomain == 'ourhome' && this.value02 == '경영리더십') {
              $('#titleArea > h2 strong, #titleArea > h1').html('');
            } else {
        ...
        .done(function () {
          ...
          if (subDomain != 'safety' && subDomain != 'ourhome') {
            if (sortData == '') {
              $.get(categoryApi, {
                'value01': sort01,
                'companyID': subDomain,
...
function viewAct(viewSeq, contentsCode) {
  ...
  $.get(useApi, {'seq': seq, 'contentsCode': contentsCode}, function (data) {
    ...
    $.each(data.contents, function () {
      ...
      if(type != 's5') {
        views += '<button type="button" style="background-color:#ffcb05;" onClick="lectureSubmit(\'1\',\'' + this.contentsCode + '\',\'' + this.contentsName.replace(/"/g, '&quot;').replace(/'/g, '&quot;') + '\',\'' + this.price + '\',\'' + studyStart + '\',\'' + studyEnd + '\',\'' + this.serviceType + '\')">' +
        '신청하기</button>';
      }
      views += '</div>';
