...
function listAct(page) {
    ...
    if (loginUserID == 'sseul0613' || loginUserID == 'dr2201' || loginUserID == 'leesunghae26' || loginUserID == 'yeram07' || loginUserLevel == '7') {
...
//		actionArea += '<option value="80">수강연동(비환급)</option>';
    ...
    actionArea += '</select>';
    if (loginUserLevel <= 8) {
        var allowUserID = ['eungmin2', 'zkfmak3785', 'dhkang', 'leesunghae26', 'dr2201',
            'yeram07', 'hh19', 'yeonseok0617'];
    ...
    var listAjax = $.get(useApi, 'page=' + page + '&list=' + listCount + '&serviceType=' + serviceType + sortData, function (data) {
        ...
        if (totalCount != 0 && loginUserLevel <= userLevel) {
            $.each(data.study, function () {
                ...
                lists += '<td>';
                lists += '<b style="color:#0780c2;cursor:pointer;" onclick="modalReTest(\'' + this.lectureOpenSeq + '\'\,\'' + this.user.userID + '\')">' + this.totalReTest + '회 (상세)</b><br/>';
                if (loginUserLevel < '5') {
