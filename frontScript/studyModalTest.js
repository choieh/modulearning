if(loginUserID == 'leehr0523'){
	var testApi = '../api/apiStudyTest_lhr.php';	
} else {
...
function certPassModal(startDate,studySeq,contentsCode,lectureOpenSeq){
	...
	var cautionApi = $.get('../api/apiSiteInfo.php',function(data){
		...
		if(motpUser.includes(loginUserID)){
			...
			studyModals += `<button class="btnPhone" type="button" onclick="captchaRun('','start','hrdOTPTest.php?contentsCode=${contentsCode}&lectureOpenSeq=${lectureOpenSeq}&type=start');">본인인증</button>&nbsp;&nbsp;`;
		...
		studyModals += '<input type="hidden" id="certContentsCode" name="contentsCode" value="'+contentsCode+'" />' //2018-04-18 본인인증 1일 1회(강혜림 추가)
        studyModals += '<input type="hidden" name="lectureOpenSeq" id="lectureOpenSeq" value="'+lectureOpenSeq+'" />'
        studyModals += '<input type="hidden" name="resultCode" id="resultCode" value="" />' //2020-04-21 추가
		studyModals += '</form>';
		studyModals += '<ul id="certMsg"><li>';
		studyModals += '<h1>본인인증</h1>';
