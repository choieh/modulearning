	...
	var writeAjax = $.get(matchingApi,function(data){
		...
		if(sortCount != 0){
				$.each(data.survey,function(){		
					...
					chapterLists += '<button type="button" onClick="contentsAct('+this.seq+',\'delete\')">삭제</button>';
					chapterLists += '</td>';
					chapterLists += '</tr>'	;
				})
			...
			$.each(data.survey, function(){
				if(action == 'copy'){
					...
					if(this.surveyType == "A"){
						...
						$('.answerA').css('display','list-item');
						$('.answerB').css('display','none');
						$('#example01').html(this.example01);
						$('#example02').html(this.example02);
						$('#example03').html(this.example03);
						$('#example04').html(this.example04);
						$('#example05').html(this.example05);
