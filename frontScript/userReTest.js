    ...
    var listAjax = $.get(useApi, function (data) {
        ...
        if (totalCount != 0) {
            $.each(data.study, function () {
                ...
                lists += '<ul>';
                if (this.leftDate == '종료') {
                    lists += '<li><h1>남은 재응시 가능일</h1><strong style="color:red;">' + this.leftDate + '</strong>일</li>';
                } else {
                    lists += '<li><h1>남은 재응시 가능일</h1><strong>' + this.leftDate + '</strong>일</li>';
                }
                if (loginUserID == 'dlwkdgml112') {
                    if (this.serviceType == '3' && this.passOK == 'Y') {	//수료시 수료증 출력
            ...
            lists += '<li class="noList">재응시 가능한 과정이 없습니다.</li>';
        }
        ...
        if (totalCount > 0) {
            ...
            $('#titleArea h3 span').html('재응시 가능한 과정이 없습니다.');
        }
...
function runStudyList(data, writeTarget) {
    ...
    //최종평가
    if (useFinTest != null && (data.serviceType == 1 || data.serviceType == 9 || data.serviceType == 5 || data.serviceType == 3)) { //2017.07.19 강혜림 서비스타입 5번 추가
        ...
        studyList += '<button type="button" title="평가응시" onclick="chkTest(\'final\',this);"><img src="../images/study/btn_dotest.png" alt="평가응시" /></button>';
...
function bbsWrite(contentsCode) {
    //강의실 과정 게시판
    $.get('../api/apiBoard.php', {
        'boardCode': '15',
        'addItem01': contentsCode
    }, function (data) {
