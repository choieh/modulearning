...
var modalStudyApi = '../api/apiStudysangmin.php';
...
var modalSendLogApi = '../api/apiSendLog.php';
	...
	if(types == 'memberView'){
		...
		$.get(modalMemberApi,{'seq':modalSeq,'userID':eachID},function(data){
			$.each(data.member,function(){
				...
				modalWrite +='<h1>상담 이력 <span style="font-size:12pt;"></span></h1>';

				modalWrite +='		<input type="hidden" name="userID" value="'+this.userID+'"/>';
				modalWrite +='		<input type="hidden" name="userName" value="'+this.userName+'"/>';
				modalWrite +='		<input type="hidden" name="phone01" value="'+this.mobile01+'"/>';
				modalWrite +='		<input type="hidden" name="phone02" value="'+this.mobile02+'"/>';
				modalWrite +='		<input type="hidden" name="phone03" value="'+this.mobile03+'"/>';
				modalWrite +='		<input type="hidden" name="adviser" value="'+loginUserID+'"/>';
				
				modalWrite +='		<div class="datePicker" style="margin-bottom: 3px;">';
				modalWrite +='			<input type="text" name="csDate" class="cal" value="" readonly="readonly" style="height: 30px;"/>';
				modalWrite +='		</div>';

				modalWrite +='		<select name="csType" style="height: 30px;">';
				modalWrite +='			<option value="login">로그인</option>';
				modalWrite +='			<option value="certi">본인인증</option>';
				modalWrite +='			<option value="test">평가관련</option>';
				modalWrite +='			<option value="etc">기타</option>';
				modalWrite +='		</select>';

				modalWrite +='		<select name="csMethod" style="height: 30px;">';
				modalWrite +='			<option value="call">전화</option>';
				modalWrite +='			<option value="question">1:1문의</option>';
				modalWrite +='			<option value="board">게시판</option>';
				modalWrite +='			<option value="etc">기타</option>';
				modalWrite +='		</select><br>';

				modalWrite +='		<input type="text" name="content" value="" style="height: 30px; width: 350px;" placeHolder="등록하실 상담 내용을 입력하세요"/>';

				modalWrite +='		<input type="text" name="actionContent" value="" style="height: 30px; width: 350px;" placeHolder="조치사항을 입력하세요"/>';
				modalWrite +='		<button type="button" onclick="csSaveAct();" style=" height: 30px; border: 1px solid #ccc; vertical-align: middle; background: #efefef; color: #565656; line-height: 23px; margin-bottom: 3px;">저장</button>';


				modalWrite +='<div class="BBSCsList scrollDiv" style="height:250px; margin:10px 0;">';

				modalWrite +='		<table>';
				modalWrite +='			<colgroup>';
				modalWrite +='				<col style="width: 80px; "/>';
				modalWrite +='				<col style="width: 80px;"/>';
				modalWrite +='				<col style=""/>';
				modalWrite +='			</colgroup>';
				modalWrite +='			<thead>';
				modalWrite +='				<tr>';
				modalWrite +='					<th>날짜</th>';
				modalWrite +='					<th>유형</th>';
				modalWrite +='					<th>상담방법</th>';
				modalWrite +='					<th>내용</th>';
				modalWrite +='					<th>조치사항</th>';
				modalWrite +='					<th style="width:60px;">수정</th>';
				modalWrite +='					<th style="width:60px;">삭제</th>';
				modalWrite +='				</tr>';
				modalWrite +='			</thead>';
				...
				//학습독려현황
				modalWrite +='<h1>학습독려현황<span style="font-size:12pt;"></span></h1>';

				modalWrite +='<div class="BBSCsList scrollDiv" style="height:250px; margin:10px 0;">';
				modalWrite +='	<table>';
				modalWrite +='		<colgroup>';
				modalWrite +='			<col style="width: 150px;"/>';
				modalWrite +='			<col style="width: 80px;"/>';
				modalWrite +='			<col style=""/>';
				modalWrite +='		</colgroup>';
				modalWrite +='		<thead>';
				...
				modalWrite +='			</tr>';
				modalWrite +='		</thead>';
				modalWrite +='		<tbody class="BBSSendLogTbody"></tbody>';
				modalWrite +='	</table>';
				modalWrite +='</div>';
				...
				$.get(modalStudyApi,{'userID':this.userID},function(data){
					...
					if(data.totalCount != 0){
						$.each(data.study,function(i){
							...
							console.log("끝");
							//상담이력 수강 콤보박스 만들기
							console.log(this.contents.contentsName);
							if(i==0){
								csComboWrite = '<select name="studySeq" style="height: 30px; width: 180px;">';
							}

							csComboWrite += '<option value="'+this.seq+'">['+this.lectureStart+'~'+this.lectureEnd+'] '+this.contents.contentsName+'</option>';
							
							if(data.study.length == (i+1)) csComboWrite += '</select>';
						})
					}else{
						listView +='<tr><td colspan="20">수강중인 과정이 없습니다.</td></tr>';
					}
					$('#modal .BBSList tbody').html(listView);
					$('select[name=csType]').before(csComboWrite); //상담이력 수강 콤보박스 추가
				})
			})
		...
		//상담이력
			...
			$.get(modalCsApi,{'userID':data.member[0].userID},function(data){
				if(data.totalCount != 0){
					...
					$.each(data.data,function(){
						...
						listCsView +='<input type="hidden" id="adviser" value="'+loginUserID+'"/>';
						var csDateView = this.years+'-'+ (this.months.length==1?'0'+this.months:this.months) +'-'+(this.days.length==1?'0'+this.days:this.days);

						listCsView +='<tr style="height: 40px;">';
						listCsView +='	<td id="csDate'+this.seq+'" style="text-align:center; display:"";">'+csDateView+'</td>';
						
						listCsView +='<td id="csDateUpdate'+this.seq+'" style="display:none;">'
						listCsView +='		<div id="datePicker'+this.seq+'" class="datePicker" style="margin-bottom: 3px; style="display:none;">';
						listCsView +='			<input type="text" id="csDateUpdateForm'+this.seq+'" class="cal" value="'+csDateView+'" readonly="readonly" style="height: 30px;"/>';
						listCsView +='		</div>';
						listCsView +='</td>'

						listCsView +='	<td id="csType'+this.seq+'" style="text-align:center; display:"";">'+this.csTypeName+'</td>';
						
						listCsView +='  <td id="csTypeUpdate'+this.seq+'" style="display:none;">';
						listCsView +='	<select id="csTypeSelect'+this.seq+'" style="height: 30px;">';
						if(this.csType == "login"){
							listCsView +='		<option value="login" selected="selected">로그인</option>';
						}else{
							listCsView +='		<option value="login">로그인</option>';
						}
						if(this.csType == "certi"){
							listCsView +='		<option value="certi" selected="selected">본인인증</option>';
						}else{
							listCsView +='		<option value="certi">본인인증</option>';
						}
						if(this.csType == "test"){
							listCsView +='		<option value="test" selected="selected">평가관련</option>';
						}else{
							listCsView +='		<option value="test">평가관련</option>';
						}
						if(this.csType == "etc"){
							listCsView +='		<option value="etc" selected="selected">기타</option>';
						}else{
						...
						listCsView +='	<td id="csMethod'+this.seq+'" style="text-align:center; display:"";">'+this.csMethod+'</td>';
						
						listCsView +='  <td id="csMethodUpdate'+this.seq+'" style="display:none;">';
						listCsView +='	<select id="csMethodSelect'+this.seq+'" style="height: 30px;">';
						if(this.csMethod == "전화"){
							listCsView +='		<option value="call" selected="selected">전화</option>';
						}else{
							listCsView +='		<option value="call">전화</option>';
						}
						if(this.csMethod == "1:1문의"){
							listCsView +='		<option value="question" selected="selected">1:1문의</option>';
						}else{
							listCsView +='		<option value="question">1:1문의</option>';
						}
						if(this.csMethod == "게시판"){
							listCsView +='		<option value="board" selected="selected">게시판</option>';
						}else{
							listCsView +='		<option value="board">게시판</option>';
						}
						if(this.csMethod == "기타"){
							listCsView +='		<option value="etc" selected="selected">기타</option>';
						}else{
						...
						listCsView +='	</select><br>';
						listCsView +='  </td>';

						listCsView +='	<td id="content'+this.seq+'" style="display:"";">'+this.content+'</td>';
						listCsView +='	<td id="updateContent'+this.seq+'" style="display:none;"><input type="text" id="updateContentForm'+this.seq+'" style="height: 30px;" value="'+this.content+'"/></td>';
						listCsView +='	<td id="actionContent'+this.seq+'" style="display:"";">'+this.actionContent+'</td>';
						listCsView +='	<td id="updateActionContent'+this.seq+'" style="display:none;"><input type="text" id="updateActionContentForm'+this.seq+'" style="height: 30px; align" value="'+this.actionContent+'"/></td>';
						

						listCsView +='	<td id="csUpdateForm'+this.seq+'" style="text-align:center; display:"";"><input type="button" value="수정" onclick="csUpdateForm('+this.seq+')" ></td>';
						listCsView +='	<td id="csUpdate'+this.seq+'" style="text-align:center; display:none;"><input type="button" value="저장" onclick="csUpdateAct('+this.seq+')" ></td>';
						
						listCsView +='	<td id="csUpdateForm'+this.seq+'" style="text-align:center; display:"";"><input type="button" value="삭제" onclick="csDelete('+this.seq+')" ></td>';
						listCsView +='</tr>';
						listCsView +='</form>';
					});
			...
			//학습독려현황
			var listSendLogView = '';
			$.get(modalSendLogApi,{'userID':data.member[0].userID},function(data){
				...
				if(data.totalCount != 0){
					...
					$.each(data.sendLog,function(){
						...
						//var csDateView = this.years+'-'+ (this.months.length==1?'0'+this.months:this.months) +'-'+ (this.months.days==1?'0'+this.days:this.days);

						listSendLogView +='<tr style="height: 40px;">';
						listSendLogView +='	<td style="text-align:center;">'+this.sendDate+'</td>';
						listSendLogView +='	<td style="text-align:center;">'+this.sendMethod+'</td>';
						listSendLogView +='	<td>'+this.sendType+'</td>';
						listSendLogView +='</tr>';
					});
				}else{
					listSendLogView +='<tr style="height: 40px; text-align: center;">';
					listSendLogView +='	<td colspan="3">등록된 학습독려 이력이 없습니다.</td>';
					listSendLogView +='</tr>';
				}

				//$('#modal .BBSCsList tbody').html(listSendLogView);
				$('.BBSSendLogTbody').html(listSendLogView);
			...
			if(data.totalCount != 0){
				$.each(data.company,function(){
					...
					if(this.studyEnabled == 'Y'){
						modalWrite +='사용&nbsp;(&nbsp;주소&nbsp;:&nbsp;<a href="http://'+this.cyberURL+'" target="_blank">'+this.cyberURL+'</a>&nbsp;)'
					}else{
						modalWrite +='사용하지 않음'
					}
					modalWrite +='</div>';
					...
					modalWrite +='<a href="'+this.siteURL+'" target="_blank">'+this.siteURL+'</a>';
					modalWrite +='</div>';
					modalWrite +='</li>';
				...
				//if(loginUserLevel < 5) {
					...
					modalWrite +='<table><thead><tr>';
					modalWrite +='<th style="width:150px">기간</th>';
	...
	//평가 결과조회
		...
		$.get(modalTestResultApi,{'lectureOpenSeq':modalSeq,'userID':eachID,'testType':option,'admin':'Y'},function(data){
			if(loginUserID == ''){
				...
				top.location.href='http://'+siteURL+'/admin/';
			}
			...
			if(data.cTypeEA != 0){
				...
				$.each(data.cType,function(){
					...
					if(this.userAnswer == null) {
						...
						var cTypeAnswer = this.userAnswer;
					}
					...
					modalWrite +='</tr>';
					modalWrite +='<tr>';
					modalWrite +='<td>점수확인</td>';
					modalWrite +='<td class="left"><input type="text" name="scoreCheck[]" value="" style="width:50px;">※ 채점한 점수를 다시 확인하여 입력해주세요.(숫자만 입력)</td>';
					modalWrite +='</tr>';
					modalWrite +='<tr>';
					...
					if(loginUserLevel == '7') {
						modalWrite +='<td class="left"><textarea name="correct[]" style="width:700px; height:200px; oncontextmenu="return false" onkeydown="keyCheck(event)" oncontextmenu="return false">'+correct+'</textarea></td>';
					} else {
						modalWrite +='<td class="left"><span style="font-size:13pt; color:red;">'+correct+'</span></td>';
					}
			...
			if(loginUserLevel == '7') {
				if(data.status == 'C') {
					...
					var todayCheck = data.nowTime;
					/*
					var todayCheck = new Date();
					var checkDateY = todayCheck.getFullYear();
					var checkDateM = todayCheck.getMonth()+1;
					var checkDateD = todayCheck.getDate();
					var checkTimeH = todayCheck.getHours();
					var checkTimeM = todayCheck.getMinutes();
					var checkTimeS = todayCheck.getSeconds();


					if (checkDateM < 10){
						checkDateM = "0" + checkDateM;
					}
					if (checkDateD < 10){
						checkDateD = "0" + checkDateD;
					}
					if (checkTimeH < 10 || checkTimeM < 10 || checkTimeS < 10){
						checkTimeH = "0" + checkTimeH;
						checkTimeM = "0" + checkTimeM;
						checkTimeS = "0" + checkTimeS;
					}
					todayCheck = checkDateY+'-'+checkDateM+'-'+checkDateD+' '+checkTimeH+':'+checkTimeM+':'+checkTimeS;
					*/
					if(data.testEndTime < todayCheck){ // 응시중인데 채점이 되는걸 막기위함
			...
			if(data.totalCount != 0){
				$.each(data.reportResult,function(){
					...
					if(this.examAttach != null) {
						modalWrite +='<br /><br /><a href="fileDownLoad.php?fileName='+this.examAttach+'&link='+this.examAttachLink+'" target="_blank">첨부파일 : '+this.examAttach;
					}
