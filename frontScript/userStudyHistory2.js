...
function ajaxAct(){
	var listAjax = $.get(useApi,function(data){
		...
		if (totalCount != 0){
			...
			lists += '<h1><img src="../images/study/img_history01.png" alt="이상에듀가 다음과정을 추천해드립니다."></h1>';
			...
			$.each(data.study, function(){
				...
				if (this.serviceType == 1 || this.serviceType == 9 || this.serviceType == 5){ // 환급 또는 테스트과정인 경우
					lists += '<li class="passOK"><h1>수료여부</h1>'+passOK+'<br/><button type="button">미수료문의</button></li>';
...
function studyPop(player,contentsCode,chapter){
	...
	window.open(popupAddress,"이상에듀학습창","top=0,left=0,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","esangStudy")
}
...
function printPop(popseq){
	$.get(useApi,{'seq':popseq,'print':'Y'},function(data){
		if(data.study[0].company[0].inputDateS || (data.study[0].serviceType == 3 && data.study[0].companyID == 'seegene') || data.study[0].serviceType == 3){	//2017-07-17 이응민 수정
			...
			alert('수강마감 (점수확인 문자 발송 다음 날) 이후에 수료증 출력이 가능합니다.');
		}
	})	
}
