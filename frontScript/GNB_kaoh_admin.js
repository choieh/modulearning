...
//	작성자 : 서영기
//
var GNBApi = '../api/apiCategoryKaoh.php'
	...
	if(type=="adminGNB"){
		...
		var GNBAction = $.get(GNBApi,{'value01':type},function(data){			
			$.each(data.category, function(){
				...
				//KSA에서 사업주정보, 커뮤니티 메뉴 노출하지 않음 2018-09-06 진영하
				if(this.enabled != 'N' && GNBFirstPermit <= loginUserLevel && GNBLastPermit >= loginUserLevel && this.seq != '89' && this.seq != '193'){
                    if(this.seq == '699' && joinURL == 'cbhy' && membership == 'N') {
                        return true;
                    }
					GNBS +='<li>';
					GNBS +='<a id="'+this.seq+'" href="'+this.value03+'"';
					if(GNBSel == GNBSelect){
						GNBS += 'class="select"';
					}
					GNBS += '>';
			...
			$('.admin_tab > .clearfix').html(GNBS);
		})
		.done(function(){
			var subPrint = $('.admin_tab > .clearfix > li > a.select').attr('id');

			//서브메뉴 출력
			var subMenuAction = $.get(GNBApi,{'seq':subPrint},function(data){
                var subMenus = '';
                if(loginUserLevel != '8'){ //학원회원관리자만 수강등록메뉴 노출
                    $.each(data.category,function(){
                        ...
                        if(this.enabled != 'N' && subFirstPermit <= loginUserLevel &&  subLastPermit >= loginUserLevel && this.seq !='701'){
                            subMenus += '<li>';
                            subMenus += '<a id="'+this.seq+'" href="'+this.value03+'"';
                    ...
                    $('.admin_sub .sub_menu ul').html(subMenus);
                    var pageTitleSeq = $('.sub_menu li a.select').attr('id')
