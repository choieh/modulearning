	...
	if(loginUserLevel <= 3) { // 관리자 모드 접속 시
		...
		*/
		...
		for(i= 2019; i <= thisYear; i++){
			contents += '<tr><td>'+i+'</td>';
			contents += '<td colspan="4"><span onclick="yearCountAJAX('+i+');" style="cursor:pointer; font-color:#000000;">확인하기 : 클릭 시 데이터를 불러옵니다.</span></td></tr>';
		}
		contents += '</tbody></table>';
		contents += '</div>';
...
function yearCountAJAX(year){ //연도별로 클릭시 해당연도의 데이터가 나오게 하도록 수정 20210114 이화랑
	var listAjax = $.get(useApi,{'year':year},function(data){
		...
		for(i= 2019; i <= thisYear; i++){
			if (totalCount != 0){
				if(year == i){
					$('#yearCount tbody').html('<tr><td>'+i+'</td><td colspan="4"><img src="../images/global/loading.gif" alt="loading" border=0 style="width:20px; height:20px;"> 데이터를 불러오는 중입니다.</td></tr>');
					lists += '<tr>';
					lists += '<td>'+i+'</td>';
					...
					lists +='<tr><td>'+i+'</td>';
					lists +='<td colspan="4"><span onclick="yearCountAJAX('+i+');" style="cursor:pointer; font-color:#000000;">확인하기 : 클릭 시 데이터를 불러옵니다.</span></td>';
					lists +='</tr>';
				}
			}else{
