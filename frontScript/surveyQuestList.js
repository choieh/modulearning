...
function listAct(page){
	...
	contents += '<th style="width:200px;">QUEST_CATE_NAME</th>';
	contents += '<th style="width:120px;">QUEST_TYPE</th>';
	contents += '<th style="width:80px;">QUEST_CONTENT</th>';
	...
	var listAjax = $.get(useApi,'page='+page+'&userLevel='+userLevel+'&list='+listCount+'&joinURL='+joinURL+sortData,function(data){
		...
		if (totalCount != 0){
			$.each(data.data,  function(){
					...
					lists += '<td>';
					if(this.QUEST_CATE_NAME == '' || this.QUEST_CATE_NAME == null){
						lists += '-'
					}else{
						lists += this.QUEST_CATE_NAME;;
					}
					lists += '</td>';
					lists += '<td>'+this.QUEST_TYPE+'</td>';
					lists += '<td>'+this.QUEST_CONTENT+'</td>';
